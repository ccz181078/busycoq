...
| (w1,(N0,N0),b1)::t1,(w2,(N0,N0),b2)::t2 =>
  if (eqb w1 w2) then
    match_side_0 t1 t2 &&& (fun t0 =>
      if b1 <=? b2 then
        Some ((w1,(b2-b1,0),b2)::t0)
      else
        Some ((w1,(0,b1-b2),b2)::t0)
    )%N
  else None
| nil,nil => Some nil
| _,_ => None
...
Definition RW_subst_c(x0 x1:N):RW->RW :=
  (fun '(w,(a0,a1),b) => (w,(0,0),a0*x0+a1*x1+b)%N).
...
| inl (BoundedConfig.Build_T l0' r0' s' sgn') =>
  ...
  | (w,a,b)::t =>
    if eqb r0 w then
    match b with
    | Npos _ => Some (inl (push l0a (push' (l0b,a,b) l1),t,s',sgn'))
    | _ => None
    end
    else None
  end
...
let '(w,(a0,a1),b):=x in
w ^^ ((N.to_nat a0)*i+(N.to_nat a1)*j+(N.to_nat b)).
...
Proof.
  ...
  - destruct sgn,sgn'; try congruence; (
      ...
      rewrite push'_spec;
      unfold to_seg;
      remember (N.to_nat a0 * i + N.to_nat a1 * j + N.to_nat (N.pos b)) as v1;
...
Lemma side_subst_c_spec i j ls x0 x1:
  to_side i j (map (RW_subst_c x0 x1) ls) = to_side (N.to_nat x0) (N.to_nat x1) ls.
Proof.
...
Lemma config_subst_c_spec i j c x0 x1:
  to_config i j ((config_subst_c x0 x1) c) = to_config (N.to_nat x0) (N.to_nat x1) c.
Proof.
  destruct c as [[[l r] s] sgn].
  cbn.
...
Proof.
  ...
  - remember (config_subst_Sj c3) as c4.
    ...
    destruct (eqb_spec (config_subst_c 0 k0 c3) c2); trivial.
    intros Hc2.
    eapply evstep_trans.
    1: apply Hc2.
    subst c2 c4 c5.
    eassert (H:_). {
      eapply acc_cond with (i:=O) (j:=(N.to_nat k0)).
      intros i j.
      specialize (Hc4 i j).
