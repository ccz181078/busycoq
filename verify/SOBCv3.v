...
Definition tm := Eval compute in (TM_from_str "1RB0RE_1RC0RA_0LD1RB_0LA1LD_0RF1RB_---0LD").
...
Definition d0 := [0;0;1].
Definition d1 := [1;1;1].
Definition d1a := [1;1] *> 0inf.
Definition L n := BinaryCounter d0 d1 d1a n.
Definition R n := [1;0]^^n *> 0inf.
...
Lemma LIncs n m:
  L n <| R (m) -->*
  L (pow2' (log2 n)) <| R ((N.to_nat (rest n))+m).
Proof.
  eapply LIncs_v1.
  - apply LInc.
  - apply RInc.
...
Proof.
  ...
  1: cbn; solve_init.
  eapply progress_nonhalt_cond with (P:=fun '(n,m) => n>=1 /\ m>=1).
  2: lia.
  intros [n m] HP.
  remember ((((2^(n*2)-1)*2+1)*2^(1+m*2)-1+1)) as v1.
  eexists (1+m+n,v1/3).
  split. 2: split; try lia; shelve.
  unfold config.
  follow10 LOv2.
  follow LIncs.
  rw_rest.
...
Definition tm := Eval compute in (TM_from_str "1RB1RE_1RC0RA_0LD1RB_0LA1LD_0RF0LB_---0LB").
...
Definition tm := Eval compute in (TM_from_str "1RB0RC_1RC0RA_0LD1RB_1RE1LD_---0RF_1LB1LC").
...
Proof.
  ...
  2: lia.
  intros [n m] HP.
  remember ((((2^(n*2)-1)*2+1)*2^(1+m*2)-1+0)) as v1.
  eexists (1+m+n,v1/3).
  split. 2: split; try lia; shelve.
  unfold config.
  follow10 LOv2.
...
Definition tm := Eval compute in (TM_from_str "1RB1RE_1RC0RA_0LD1RB_0LA1LD_1RF0LB_---0LD").
...
Definition tm := Eval compute in (TM_from_str "1RB0LC_1LC0RD_1LA1RC_1RE1RC_---0RF_0RB0RB").

Notation "c -->* c'" := (c -[ tm ]->* c') (at level 40).
Notation "c -->+ c'" := (c -[ tm ]->+ c') (at level 40).

Notation "l <| r" :=
  (l <{{A}} [1;0;1;1;0] *> r) (at level 30).

Notation "l |> r" :=
  (l <* <[0;1;1;0;1] {{C}}> r) (at level 30).


Definition d0 := <[0;1;0;0].
Definition d1 := <[0;1;0;1].
Definition L n := BinaryCounter d0 d1 (0inf<*[1]) n.
Definition R n := [1]^^n *> 0inf.
...
Lemma LOv0 n m:
  L (pow2' (1+n)) <| R (0+m*3) -->+
  L ((((pow2 n)~1)*(pow2 m))~0) <| R 2.
Proof.
  unfold L,R.
  rw_BinaryCounter.
  unfold d0,d1.
  es. er.
  epose proof (Incs0 _ O m) as H.
  follow H. clear H.
  es.
Qed.
...
Definition config0 n m :=
  L (pow2' (1+n)) <| R m.

Lemma BigStep0 n m:
  config0 n (0+m*3) -->+
  config0 (1+m+n) ((((2^n-1)*2+1)*2^m-1)*2+3).
Proof.
  unfold config0.
  follow10 LOv0.
  follow_LIncs.
...
match x with
| c11 n m => config0 (n*2+1) (0+(m*2+1)*3)
| c10 n m => config0 (n*2+1) (1+(m*2+0)*3)
| c00 n m => config0 (n*2+0) (1+(m*2+0)*3)
end.
...
Lemma nonhalt: ~halts tm c0.
  eapply multistep_nonhalt with (c':=config (c10 1 2)).
  1: apply init.
  eapply progress_nonhalt_cond with (P:=P).
  2: unfold P; lia.
  intros [n m|n m|n m] HP; unfold P in HP.
  - remember (1 + (m * 2 + 1) + (n * 2 + 1)) as n'.
    remember ((((2 ^ (n * 2 + 1) - 1) * 2 + 1) * 2 ^ (m * 2 + 1) - 1) * 2 + 3) as m'.
    exists (c10 (n'/2) (m'/6)).
    split.
    2: unfold P; shelve.
    unfold config.
    ...
    + rewrite Nat.mul_comm.
      applys_eq Nat.Div0.div_mod; f_equal.
      subst n'.
      rw_mod; reflexivity.
    + replace (1+(m'/6*2+0)*3) with (6*(m'/6)+1) by lia.
      applys_eq Nat.Div0.div_mod; f_equal.
      subst m'.
      rw_mod; reflexivity.
  - remember (1 + (m * 2 + 0) + (n * 2 + 1)) as n'.
    remember ((((2 ^ (n * 2 + 1) - 1) * 2 + 1) * 2 ^ (m * 2 + 0) - 1) * 2 + 3) as m'.
    exists (c00 (n'/2) (m'/6)).
    split.
    2: unfold P; shelve.
    unfold config.
    ...
    + replace (1+(m'/6*2+0)*3) with (6*(m'/6)+1) by lia.
      ...
      subst m'.
      rw_mod; reflexivity.
  - remember (1 + (m * 2 + 0) + (n * 2 + 0)) as n'.
    remember ((((2 ^ (n * 2 + 0) - 1) * 2 + 1) * 2 ^ (m * 2 + 0) - 1) * 2 + 3) as m'.
    exists (c11 (n'/2) (m'/6)).
    split.
    2: unfold P; shelve.
    unfold config.
    ...
    + rewrite Nat.mul_comm.
      ...
      subst n'.
      rw_mod; reflexivity.
    + replace (0+(m'/6*2+1)*3) with (6*(m'/6)+3) by lia.
      applys_eq Nat.Div0.div_mod; f_equal.
      subst m'.
      rw_mod; reflexivity.
Unshelve.
all: try solve_ge.
all: split; subst; try solve_ge.
Qed.
...
Definition tm := Eval compute in (TM_from_str "1RB0RC_1RC0RA_0LD1RB_1RF0LE_1LD1RE_---1RA").
