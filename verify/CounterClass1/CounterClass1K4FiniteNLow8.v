...
Definition k4n8_c_lov3 (t:K4NTable) (a b:N) : option K4NResult :=
      ...
      | Some (c0,d0) =>
          ...
          | Some d0' =>
              ...
              | Some (c,z0) =>
                  ...
                  | Some z =>
                      if (c=?0) && N.even z then Some (N.div2 z,8)
                      else None
                  | None => None
                  end
              | None => None
...
Definition k4n8_c_lov4 (t:K4NTable) (a b:N) : option K4NResult :=
  match k4n_sub b 5 with
  | Some b' =>
      match k4n_get t a b' with
      | Some (c0,d0) =>
...
Definition k4n8_c_lov5 (t:K4NTable) (a b:N) : option K4NResult :=
  match k4n_sub b 5 with
  | Some b' =>
      match k4n_get t a b' with
      | Some (c0,d0) =>
...
Lemma k4n8_c_lov3_sound a b c d:
  k4n8_c_lov3 t a b=Some (c,d) ->
...
Lemma k4n8_c_lov4_sound a b c d:
  k4n8_c_lov4 t a b=Some (c,d) ->
  P (N.to_nat a) (N.to_nat b) (N.to_nat c) (N.to_nat d).
Proof.
  unfold k4n8_c_lov4.
  destruct (k4n_sub b 5) as [b'|] eqn:Eb; try discriminate.
...
Lemma k4n8_c_lov5_sound a b c d:
  k4n8_c_lov5 t a b=Some (c,d) ->
  P (N.to_nat a) (N.to_nat b) (N.to_nat c) (N.to_nat d).
Proof.
  unfold k4n8_c_lov5.
  destruct (k4n_sub b 5) as [b'|] eqn:Eb; try discriminate.
...
Definition k4n8_cell (t:K4NTable) (a b:N) : option K4NResult :=
  ...
  (k4n_or_else (k4n8_c_lov3 t a b)
  (k4n_or_else (k4n8_c_lov4 t a b)
                (k4n8_c_lov5 t a b)))))))))))).

Lemma k4n8_cell_sound P (C:K4ComplexRules P K4Low8)
    (Hrst:P 0%nat 1%nat 0%nat 5%nat) t:
  K4NTableSound P t -> forall a b c d,
  k4n8_cell t a b=Some (c,d) ->
  P (N.to_nat a) (N.to_nat b) (N.to_nat c) (N.to_nat d).
Proof.
  intros Ht a b c d. unfold k4n8_cell,k4n_or_else.
  ...
  destruct (k4n8_c_lov3 t a b) as [[x y]|] eqn:E10.
  { intros H; inverts H. eapply k4n8_c_lov3_sound; eauto. }
  destruct (k4n8_c_lov4 t a b) as [[x y]|] eqn:E11.
  { intros H; inverts H. eapply k4n8_c_lov4_sound; eauto. }
  eapply k4n8_c_lov5_sound; eauto.
Qed.

Definition k4n8_layer_step (src:K4NTable) (w:N)
    (s:N*K4NTable) : N*K4NTable :=
  let '(a,out):=s in
  let b:=w-2*a in
  let out':=
    match k4n8_cell src a b with
    | Some v => K4NMap.hmap_set (a,b) v out
    | None => out
    end in
  (N.succ a,out').

Definition k4n8_make_layer (src:K4NTable) (w:N) : K4NTable :=
  snd (N.iter (N.succ (N.div2 w)) (k4n8_layer_step src w) (0,src)).

Definition k4n8_build_step (s:N*K4NTable) : N*K4NTable :=
  let '(w,t):=s in (N.succ w,k4n8_make_layer t w).

Definition k4n8_build (last:N) : K4NTable :=
  snd (N.iter (N.succ last) k4n8_build_step
    (0,K4NMap.hmap_make k4n_table_size)).

Lemma k4n8_layer_step_sound P (C:K4ComplexRules P K4Low8)
    (Hrst:P 0%nat 1%nat 0%nat 5%nat) src w s:
  K4NTableSound P src -> K4NTableOK P (snd s) ->
  K4NTableOK P (snd (k4n8_layer_step src w s)).
Proof.
  intros Hsrc Hout. destruct s as [a out]. cbn [k4n8_layer_step] in *.
  destruct (k4n8_cell src a (w-2*a)) as [[c d]|] eqn:E; [|exact Hout].
  eapply k4n_set_sound; [exact Hout|].
  eapply k4n8_cell_sound; eauto.
Qed.

Lemma k4n8_make_layer_sound P (C:K4ComplexRules P K4Low8)
    (Hrst:P 0%nat 1%nat 0%nat 5%nat) src w:
  K4NTableOK P src -> K4NTableOK P (k4n8_make_layer src w).
Proof.
  intros Hsrc. unfold k4n8_make_layer.
  eapply (k4n_iter_preserve (fun s=>K4NTableOK P (snd s)));
    [exact Hsrc|].
  intros s Hs. eapply k4n8_layer_step_sound; eauto. exact (proj2 Hsrc).
Qed.

Lemma k4n8_build_step_sound P (C:K4ComplexRules P K4Low8)
    (Hrst:P 0%nat 1%nat 0%nat 5%nat) s:
  K4NTableOK P (snd s) -> K4NTableOK P (snd (k4n8_build_step s)).
Proof.
  destruct s as [w t]. cbn [k4n8_build_step].
  intros H. eapply (@k4n8_make_layer_sound P C Hrst). exact H.
Qed.

Lemma k4n8_build_sound P (C:K4ComplexRules P K4Low8)
    (Hrst:P 0%nat 1%nat 0%nat 5%nat) last:
  K4NTableOK P (k4n8_build last).
Proof.
  unfold k4n8_build.
  eapply (k4n_iter_preserve (fun s=>K4NTableOK P (snd s))).
  - apply k4n_make_sound.
  - intros s Hs. eapply k4n8_build_step_sound; eauto.
Qed.

Definition k4n_low8_780:=k4n8_build 780.

Lemma k4n_low8_780_check:k4n_prebase_check k4n_low8_780=true.
Proof. native_compute. reflexivity. Qed.

Lemma k4n_low8_prebase_data P (C:K4ComplexRules P K4Low8)
    (Hrst:P 0%nat 1%nat 0%nat 5%nat):
  K4ScanData P K4T2 k4_prebase_bits 382 2 378 4.
Proof.
  exact (@k4n_prebase_check_sound P k4n_low8_780
    (proj2 (@k4n8_build_sound P C Hrst 780)) k4n_low8_780_check).
Qed.
