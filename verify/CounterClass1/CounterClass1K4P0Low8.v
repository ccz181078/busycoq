Require Import BusyCoq.CounterClass1.CounterClass1Common BusyCoq.CounterClass1.CounterClass1K4Particles
  BusyCoq.CounterClass1.CounterClass1K4Words BusyCoq.CounterClass1.CounterClass1K4Phase BusyCoq.CounterClass1.CounterClass1K4BoundaryLow8
  BusyCoq.CounterClass1.CounterClass1K4CycleLow8 BusyCoq.CounterClass1.CounterClass1K4P0RulesLow8.
...
Variable C: K4ComplexRules P K4Low8.
Variable R0: K4P0Rules8 P P0.
Let R:=k4c_simple C.

Lemma k4_low8_type4_align B H S D:
  2<=H -> length B=H+1 -> nth H B false=true ->
  nth (H-1) B false=true ->
  K4Type4Front8 P B H S D -> P (H+1) 4 (H+1) 8 ->
  P0 (H+1) 8 -> P0 (H-2) 26.
Proof.
  intros HH Hlen BH BHm [Hhs [RT [RF [E1 [E2 [E3 [E4 E5]]]]]]]
    A HP0.
  destruct RT as [dt [Hdt RT]]. assert (dt=11) by lia. subst dt.
  ...
  assert (H15: P H 15 (H-1) 21) by
    (applys_eq (k4_rov' R H 13 H 13 (H-1) 17);
      try lia; applys_eq H13; lia).
  assert (Hm19: P (H-1) 19 (H-2) 25).
  { applys_eq (k4_rov' R (H-1) 17 (H-1) 17 (H-2) 21); try lia.
    - exact Hm17.
    - applys_eq Hm17; lia. }
  assert (P1:P0 H 12) by
    (applys_eq (k4p08_ov' R0 (H+1) 4 H 8); try lia;
      [applys_eq HP0|applys_eq A]; lia).
  assert (P2:P0 (H+1) 12) by
    (applys_eq (k4p08_ov R0 H 9 (H+1) 11); try lia;
      [applys_eq P1|applys_eq TH]; lia).
  assert (P3:P0 H 16) by
    (applys_eq (k4p08_ov' R0 (H+1) 8 H 12); try lia;
      [applys_eq P2|applys_eq E1]; lia).
  assert (P4:P0 H 18) by
    (applys_eq (k4p08_ov R0 H 13 H 17); try lia;
      [applys_eq P3|applys_eq H13]; lia).
  assert (P5:P0 (H-1) 22) by
    (applys_eq (k4p08_ov R0 H 15 (H-1) 21); try lia;
      [applys_eq P4|applys_eq H15]; lia).
  applys_eq (k4p08_ov R0 (H-1) 19 (H-2) 25); try lia;
    [applys_eq P5|applys_eq Hm19]; lia.
Qed.

Lemma k4_low8_type2_align B H S D:
  2<=H -> length B=H+1 -> nth H B false=true ->
  D<length B -> D<>H-1 -> nth D B false=true ->
  K4Type2Front8 P B H S D ->
  P0 (H+3) 1 -> P0 (H-1) 21.
Proof.
  ...
  assert (P1: P0 (H+2) 5) by
    (applys_eq (k4p08_inc00_1 R0 (H+2)); try lia; applys_eq HP0; lia).
  assert (P2: P0 (H+2) 7) by
    (applys_eq (k4p08_ov R0 (H+2) 2 (H+2) 6); try lia; assumption).
  assert (P3: P0 (H+1) 11) by
    (applys_eq (k4p08_ov R0 (H+2) 4 (H+1) 10); try lia; assumption).
  assert (P4: P0 (H+1) 13) by
    (applys_eq (k4p08_ov R0 (H+1) 8 (H+1) 12); try lia; assumption).
  assert (P5: P0 H 17) by
    (applys_eq (k4p08_ov R0 (H+1) 10 H 16); try lia; assumption).
  applys_eq (k4p08_ov' R0 H 13 (H-1) 17); try lia.
  - exact P5.
  - applys_eq H13; lia.
Qed.
...
(* The one fact used before the ordinary T4 prefix is retained here rather
   ...
   end-fan and consumed immediately by [k4_low8_type4_align]. *)
Lemma k4_low8_end_t4_entry B H S D:
  3<=S -> length B=H+1 -> nth 0 B false=true ->
  nth (D+2) B false=true ->
  K4EndFan P K4T4 B H S D -> P (H+1) 4 (H+1) 8.
Proof.
  intros HS Hlen B0 BD2
  ...
  { applys_eq (k4_rov R (H+1) 0 (D+2) (4*S-1) (H+3) 1);
      try lia; [applys_eq C1; lia|applys_eq L2; lia]. }
  applys_eq (k4_inc00_2 R (H+1) 2 (H+1)); try lia.
  applys_eq Q1; lia.
Qed.

Lemma k4_low8_scan_next_t4_p0 runs B H S D k:
...
Proof.
  ...
  destruct (k4_low8_scan_next_t4 P C runs B H S D k HS HB) as [HS' HB'].
  ...
  assert (HF: K4Type2Front8 P (k4_extend B (D+3) (2*S+k+3))
      (2*H+k+7) (2*S+k+3) (2*D+1-k)).
  { apply (k4_low8_end_t2_front P C); try assumption; try lia;
      try exact (proj1 HB'); apply Hwin; lia. }
  assert (HR: P0 (2*H+k+10) 1).
  { apply (k4_p08_scan_t4_reset P P0 R R0 B H S D k); try assumption.
    exact (k4_scan_start_data P K4T4 runs B H S D k HS). }
  split; [exact HS'|]. split; [exact HB'|].
  eapply (k4_low8_type2_align (k4_extend B (D+3) (2*S+k+3))
    (2*H+k+7) (2*S+k+3) (2*D+1-k)).
  - lia.
  - applys_eq Hlen; lia.
  - exact BH.
...
Lemma k4_low8_scan_next_t2_p0 runs B H S D k:
...
Proof.
  intros HS HB HP0. cbv zeta.
  destruct (k4_low8_scan_next_t2 P C runs B H S D k HS HB) as [HS' HB'].
  ...
  assert (HF: K4Type4Front8 P (k4_extend B (D+3) (2*S+k+3))
      (2*H+k+7) (2*S+k+3) (2*D+1-k)).
  { apply (k4_low8_end_t4_front P C); try assumption; try lia;
      try exact (proj1 HB'); apply Hwin; lia. }
  assert (HA: P (2*H+k+7+1) 4 (2*H+k+7+1) 8).
  { apply (k4_low8_end_t4_entry (k4_extend B (D+3) (2*S+k+3))
      (2*H+k+7) (2*S+k+3) (2*D+1-k)); try assumption; try lia.
    - exact (proj1 HB').
    - apply Hwin; lia. }
  assert (HR: P0 (2*H+k+8) 8).
  { apply (k4_p08_scan_t2_reset P P0 R R0 B H S D k); try assumption.
    exact (k4_scan_start_data P K4T2 runs B H S D k HS). }
  split; [exact HS'|]. split; [exact HB'|].
  eapply (k4_low8_type4_align (k4_extend B (D+3) (2*S+k+3))
    (2*H+k+7) (2*S+k+3) (2*D+1-k)).
  - lia.
  - applys_eq Hlen; lia.
  - exact BH.
...
Definition K4P0Aligned8 (kind:K4Kind) (H:nat) : Prop :=
  match kind with
  | K4T4 => P0 (H-2) 26
  | K4T2 => P0 (H-1) 21
  end.

Definition K4P0StateValid8 (s:K4ScanState8) : Prop :=
  K4ScanState8Valid8 P s /\ K4P0Aligned8 (k4s8_kind s) (k4s8_H s).

Lemma k4_low8_p0_state_step s:
  K4P0StateValid8 s -> K4P0StateValid8 (k4_scan_state8_step s).
Proof.
  destruct s as [kind runs B H S D k].
  unfold K4P0StateValid8,K4ScanState8Valid8,K4P0Aligned8.
  destruct kind; cbn; intros [[HS HB] HP0].
  - destruct (k4_low8_scan_next_t4_p0 runs B H S D k HS HB HP0)
      as [HS' [HB' HP']]. split; [split; assumption|exact HP'].
  - destruct (k4_low8_scan_next_t2_p0 runs B H S D k HS HB HP0)
      as [HS' [HB' HP']]. split; [split; assumption|exact HP'].
Qed.

Lemma k4_low8_p0_base:
  K4ScanData P K4T2 k4_prebase_bits 382 2 378 4 ->
  P0 381 21 -> K4P0StateValid8 k4_low8_base_state.
Proof.
  intros HD HP0. split; [exact (k4_low8_base_valid P C HD)|].
  cbn [K4P0Aligned8].
  pose proof (k4_p08_scan_t2_reset P P0 R R0 k4_prebase_bits
  ...
  assert (HF: K4Type4Front8 P k4_base_bits 775 11 753).
  { apply (k4_low8_end_t4_front P C); try assumption; try lia;
      try exact (proj1 k4_base_head_bits); apply Hwin; lia. }
  assert (HA: P 776 4 776 8).
  { apply (k4_low8_end_t4_entry k4_base_bits 775 11 753).
    - lia.
    - exact Hlen.
    - exact (proj1 k4_base_head_bits).
    - apply Hwin; lia.
    - exact HE. }
  eapply (k4_low8_type4_align k4_base_bits 775 11 753).
  - lia.
  - exact Hlen.
  - exact BH.
  - exact BHm.
...
Lemma k4_low8_p0_cycle
    (HD:K4ScanData P K4T2 k4_prebase_bits 382 2 378 4)
    (HP0:P0 381 21) n:
  K4P0StateValid8 (k4_scan_state8_iter n k4_low8_base_state).
Proof.
  induction n; cbn.
  - exact (k4_low8_p0_base HD HP0).
  - apply k4_low8_p0_state_step. exact IHn.
Qed.

Lemma k4_low8_p0_cycle_config
    (HD:K4ScanData P K4T2 k4_prebase_bits 382 2 378 4)
    (HP0:P0 381 21) n:
  exists a b, P0 a b /\
    k4s8_H (k4_scan_state8_iter n k4_low8_base_state)<=a+b.
Proof.
  pose proof (k4_low8_p0_cycle HD HP0 n) as [_ HA].
  remember (k4_scan_state8_iter n k4_low8_base_state) as s.
  destruct s as [kind runs B height S D k]. cbn in *.
  destruct kind; cbn [K4P0Aligned8] in HA.
