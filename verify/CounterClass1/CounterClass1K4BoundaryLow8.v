...
Definition K4Type4Front8 (P:nat->nat->nat->nat->Prop)
    (B:list bool) (H S D:nat) : Prop :=
  H=2*S+D /\
  K4Row P B true (2*H+9) (H+1) /\
  K4Row P B false (2*H+10) (H+1) /\
  P (H+1) 8 (H+1) 12 /\
  P (H+2) 6 (H-1) 16 /\
  P (H+3) 4 H 14 /\
  P (H+4) 2 (H+1) 12 /\
...
Definition K4Type2Front8 (P:nat->nat->nat->nat->Prop)
    (B:list bool) (H S D:nat) : Prop :=
  H=2*S+D /\
  K4Row P B false (2*H+6) (H+2) /\
  P (H+1) 4 (H+1) 8 /\
  P (H+2) 2 (H+2) 6 /\
  P (H+3) 0 D (4*S+10) /\
...
Lemma k4_type4_front8_pre P B H S D:
  K4Type4Front8 P B H S D -> K4Type4PreFront P B H S D.
Proof. intros [? [? [? [? [? [? [? ?]]]]]]]; repeat split; assumption. Qed.

Lemma k4_type2_front8_pre P B H S D:
  K4Type2Front8 P B H S D -> K4Type2PreFront P B H S D.
Proof. intros [? [? [? [? [? [? ?]]]]]]; repeat split; assumption. Qed.
...
Proof.
  ...
  { exists 7. split; try lia. intros a b Hal Han Hab Haw.
    ...
    - applys_eq (k4_inc00_1 R a (b-4) (H+2)); try lia.
      ...
      + applys_eq Z; lia.
    - exact E2. }
  assert (F4:K4ExactRow P B false (2*H+4) H 8).
  { split; try lia. intros a b Hal Hab Haw.
    ...
    - applys_eq Z; lia. }
  assert (FR:K4ExactRow P B false (2*H+6) (H+2) 6).
  { applys_eq (k4_exact_row_rov P R B false (2*H+4)
      H 5 (H+2) 5); try lia.
    - exact F4.
    - exact IH. }
  unfold K4Type2Front8. repeat split; try assumption.
  exists 6. exact FR.
Qed.

Lemma k4_low8_type2_scan_rows B H S D:
...
Proof.
  intros HH Hlen BH HDl HDn BD BHm
    [Hhs [RF0 [E1 [E2 [E3 [RT0 Dent]]]]]].
  destruct RF0 as [df [Hdf RF0]]. destruct RT0 as [dt [Hdt RT0]].
  assert (df=6) by lia. assert (dt=7) by lia. subst df dt.
  ...
  assert (B3:P (H+1) 10 H 16) by
    (applys_eq (k4_rov' R (H+1) 8 (H+1) 8 H 12); try lia;
      applys_eq B2; lia).
  assert (RF1:K4ExactRow P B false (2*H+8) (H+1) 10).
  { applys_eq (k4_exact_row_rov' P R B false (2*H+6)
      (H+2) 2 (H+1) 6); try lia.
    - split; assumption.
    - applys_eq E2; lia. }
  assert (RF2:K4ExactRow P B false (2*H+10) (H-1) 16).
  { applys_eq (k4_exact_row_rov' P R B false (2*H+8)
      (H+1) 6 (H-1) 12); try lia.
    - exact RF1.
    - applys_eq B1; lia. }
  ...
  assert (RF3:K4ExactRow P B false (2*H+12) (H+1) 14) by
    (applys_eq (k4_exact_row_rov P R B false (2*H+10)
      (H-1) 13 (H+1) 13); try lia; assumption).
  ...
  assert (RF4:K4ExactRow P B false (2*H+14) (H-1) 20).
  ...
  assert (RF5:K4ExactRow P B false (2*H+16) (H-1) 22) by
    (applys_eq (k4_exact_row_rov P R B false (2*H+14)
      (H-1) 17 (H-1) 21); try lia; assumption).
  split; assumption.
Qed.

Lemma k4_low8_end_t4_front B H S D:
  ...
  K4EndFan P K4T4 B H S D -> K4Type4Front8 P B H S D.
Proof.
  ...
  assert (F6:K4ExactRow P B false (2*H+6) (H+1) 8).
  { split; try lia. intros a b Hal Hab Haw.
    assert (a<>H) by (intros ->; congruence).
    assert (7<=b) by lia.
    assert (Fa:P a (b-7) 0 (2*H+3)) by
      (apply RF; assumption || lia).
    assert (Fa1:P a (b-5) 0 (2*H+5)).
    { applys_eq (k4_rov R a (b-7) 0 (2*H) 0 (2*H+4));
        try lia; [applys_eq Fa|applys_eq ZT]; lia. }
    applys_eq (k4c_lov4 C a (b-5) 0 (2*H) 0 (2*H) (H+1));
      cbn; try lia; [applys_eq Fa1|applys_eq ZT|applys_eq ZT]; lia. }
  assert (F8:K4ExactRow P B false (2*H+8) H 12).
  { applys_eq (k4_exact_row_rov' P R B false (2*H+6)
      (H+1) 4 H 8); try lia.
    - exact F6.
    - applys_eq E1; lia. }
  assert (FR:K4ExactRow P B false (2*H+10) (H+1) 12).
  { applys_eq (k4_exact_row_rov P R B false (2*H+8)
      H 9 (H+1) 11); try lia; assumption. }
  unfold K4Type4Front8. repeat split; try assumption.
  - exists 11. exact TR.
  - exists 12. exact FR.
Qed.

Lemma k4_low8_type4_scan_rows B H S D:
  2<=H -> length B=H+1 ->
  nth H B false=true -> nth (H-1) B false=true ->
  K4Type4Front8 P B H S D ->
  K4ExactRow P B true (2*H+17) (H-2) 25 /\
  K4ExactRow P B false (2*H+18) (H-2) 26.
Proof.
  intros HH Hlen BH BHm
    [_ [RT [RF [E1 [_ [_ [_ _]]]]]]].
  destruct RT as [dt [Hdt RT]]. destruct RF as [df [Hdf RF]].
  assert (dt=11) by lia. assert (df=12) by lia. subst dt df.
  ...
  { applys_eq (k4_exact_row_rov P R B true (2*H+9)
    ...
    - split; assumption.
    - exact E1. }
  assert (RF1:K4ExactRow P B false (2*H+12) H 16).
  { applys_eq (k4_exact_row_rov' P R B false (2*H+10)
      (H+1) 8 H 12); try lia.
    - split; assumption.
    - applys_eq E1; lia. }
  ...
  assert (RF2:K4ExactRow P B false (2*H+14) H 18).
  { applys_eq (k4_exact_row_rov P R B false (2*H+12)
...
Lemma k4_low8_type4_exception B H S D:
  6<=D -> K4Type4Front8 P B H S D ->
  P (H+9) 0 (D-6) (4*S+34).
Proof.
...
Lemma k4_low8_end_t4_payload B H S D:
...
Proof.
  ...
  assert (HF:K4Type4Front8 P B H S D).
  { apply (k4_low8_end_t4_front B H S D); try assumption; try lia;
      apply Hwin; lia. }
  destruct (k4_low8_type4_scan_rows B H S D ltac:(lia)
    Hlen BH BHm HF) as [RT RF].
  assert (Hpre:K4Type4PreTail P H S D).
  { apply (k4_type4_pretail P R B H S D); try assumption; try lia.
    - intros z Hz. apply Hwin. lia.
    - exact (k4_type4_front8_pre P B H S D HF). }
  ...
  assert (Hex:P (H+9) 0 (D-6) (4*S+34)) by
    (apply (k4_low8_type4_exception B H S D); assumption).
...
Lemma k4_low8_type2_exception B H S D:
  5<=D -> K4Type2Front8 P B H S D ->
  P (H+8) 0 (D-5) (4*S+30).
Proof.
...
Lemma k4_low8_end_t2_payload B H S D:
...
Proof.
  intros HS HD Hlen B0 BH BHm Hwin HE.
  assert (Hhs:H=2*S+D) by exact (proj1 HE).
  assert (HF:K4Type2Front8 P B H S D).
  { apply (k4_low8_end_t2_front B H S D); try assumption;
      apply Hwin; lia. }
  destruct (k4_low8_type2_scan_rows B H S D ltac:(lia) Hlen BH
    ltac:(lia) ltac:(lia) ltac:(apply Hwin; lia) BHm HF) as [RT RF].
  assert (Hpre:K4Type2PreTail P H S D).
  { apply (k4_type2_pretail P R B H S D); try assumption; try lia.
    - intros z Hz. apply Hwin. lia.
    - exact (k4_type2_front8_pre P B H S D HF). }
  ...
  assert (Hex:P (H+8) 0 (D-5) (4*S+30)) by
    (apply (k4_low8_type2_exception B H S D); assumption).
  unfold K4ScanPayload; cbn.
  split; [applys_eq RT; lia|]. split; [applys_eq RF; lia|].
  split; [intros j Hj; applys_eq (Htail j Hj); lia|].
  applys_eq Hex; lia.
