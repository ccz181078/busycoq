Require Import BusyCoq.CounterClass1.CounterClass1Common BusyCoq.CounterClass1.CounterClass1K4Particles BusyCoq.CounterClass1.CounterClass1K4Phase
  BusyCoq.CounterClass1.CounterClass1K4Words BusyCoq.CounterClass1.CounterClass1K4Boundary BusyCoq.CounterClass1.CounterClass1K4BoundaryLow4.
Require Import Lia.
...
Lemma k4_low4_prebase_to_base_bits:
  k4_extend k4_prebase_bits 381 11=k4_base_bits.
Proof. vm_compute. reflexivity. Qed.

Lemma k4_low4_scan_next_t4 runs B H S D k:
...
Proof.
  ...
  { apply (k4_low4_end_t2_payload P C); try assumption; try lia.
...
Lemma k4_low4_scan_next_t2 runs B H S D k:
...
Proof.
  ...
  assert (HP: K4ScanPayload P K4T4 (k4_extend B (D+3) (2*S+k+3))
      (2*H+k+7) (2*S+k+3) (2*D+1-k)).
  { apply (k4_low4_end_t4_payload P C); try assumption; try lia.
    exact (proj1 Hheadbits). }
  split; [|exact Hheadbits].
...
Proof.
  ...
  change (K4EndFan P K4T4 (k4_extend k4_prebase_bits 381 11)
    775 11 753) in HE.
  rewrite k4_low4_prebase_to_base_bits in HE.
...
Definition k4_scan_state4_step (s:K4ScanState4) : K4ScanState4 :=
  {| k4s4_kind := k4_next_kind (k4s4_kind s);
     k4s4_runs := k4s4_runs s ++ [k4s4_D s+3;1;2*k4s4_S s+k4s4_k s+3];
     k4s4_bits := k4_extend (k4s4_bits s) (k4s4_D s+3)
       (2*k4s4_S s+k4s4_k s+3);
     k4s4_H := 2*k4s4_H s+k4s4_k s+7;
     k4s4_S := 2*k4s4_S s+k4s4_k s+3;
     k4s4_D := 2*k4s4_D s+1-k4s4_k s;
     k4s4_k := k4_next_k (k4s4_kind s) (k4s4_k s) |}.

Fixpoint k4_scan_state4_iter (n:nat) (s:K4ScanState4) : K4ScanState4 :=
  match n with
  | 0 => s
  | S n => k4_scan_state4_step (k4_scan_state4_iter n s)
  end.

Definition k4_low4_base_state : K4ScanState4 :=
  {| k4s4_kind := K4T4;
     k4s4_runs := k4_base_runs;
     k4s4_bits := k4_base_bits;
     k4s4_H := 775;
     k4s4_S := 11;
     k4s4_D := 753;
     k4s4_k := 6 |}.

Definition K4ScanState4Valid4 (s:K4ScanState4) : Prop :=
  K4ScanStart P (k4s4_kind s) (k4s4_runs s) (k4s4_bits s)
    (k4s4_H s) (k4s4_S s) (k4s4_D s) (k4s4_k s) /\
  K4HeadBits (k4s4_kind s) (k4s4_bits s).

Lemma k4_low4_state_step s:
  K4ScanState4Valid4 s -> K4ScanState4Valid4 (k4_scan_state4_step s).
Proof.
  destruct s as [kind runs B H S D k]. unfold K4ScanState4Valid4.
  destruct kind; cbn; intros [HS HB].
  - exact (k4_low4_scan_next_t4 runs B H S D k HS HB).
  - exact (k4_low4_scan_next_t2 runs B H S D k HS HB).
Qed.

Lemma k4_low4_base_valid:
  K4ScanData P K4T2 k4_prebase_bits 382 2 378 4 ->
  K4ScanState4Valid4 k4_low4_base_state.
Proof.
  intros H. exact (k4_low4_prebase_to_base H).
Qed.

Lemma k4_low4_cycle
    (Hpre:K4ScanData P K4T2 k4_prebase_bits 382 2 378 4) n:
  K4ScanState4Valid4 (k4_scan_state4_iter n k4_low4_base_state).
Proof.
  induction n; cbn.
  - exact (k4_low4_base_valid Hpre).
  - apply k4_low4_state_step. exact IHn.
Qed.

Lemma k4_low4_cycle_height
    (Hpre:K4ScanData P K4T2 k4_prebase_bits 382 2 378 4) n:
  775+n <= k4s4_H (k4_scan_state4_iter n k4_low4_base_state).
Proof.
  induction n; cbn; lia.
Qed.

Lemma k4_low4_cycle_left_row
    (Hpre:K4ScanData P K4T2 k4_prebase_bits 382 2 378 4) n:
  exists b c d,
    P 0 b c d /\
    k4s4_H (k4_scan_state4_iter n k4_low4_base_state)
      <= c+2.
Proof.
  remember (k4_scan_state4_iter n k4_low4_base_state) as s.
  pose proof (k4_low4_cycle Hpre n) as [HS HB]. rewrite <-Heqs in HS,HB.
...
Lemma k4_low4_cycle_left_row2
    (Hpre:K4ScanData P K4T2 k4_prebase_bits 382 2 378 4) n:
  exists b c d,
    P 2 b c d /\
    k4s4_H (k4_scan_state4_iter n k4_low4_base_state) <= c+2.
Proof.
  remember (k4_scan_state4_iter n k4_low4_base_state) as s.
  pose proof (k4_low4_cycle Hpre n) as [HS HB]. rewrite <-Heqs in HS,HB.
  pose proof (k4_scan_start_data P _ _ _ _ _ _ _ HS) as HD.
  ...
  - destruct HD as [Hlen [Hhs [Hlarge [Hk [Hs [Hbase [Hdrop
    ...
    destruct (nth 2 B false) eqn:E.
    + exists (2*H+13),(H-2),25. split.
      * applys_eq (RT 2 (2*H+13)); try assumption; lia.
      * lia.
    + exists (2*H+14),(H-2),26. split.
      * applys_eq (RF 2 (2*H+14)); try assumption; lia.
      * lia.
  - destruct HD as [Hlen [Hhs [Hlarge [Hk [Hs [Hbase [Hdrop
      [RT [RF [Htail Hex]]]]]]]]]].
    cbn [k4_gap] in RT,RF.
    destruct RT as [_ RT]. destruct RF as [_ RF].
    destruct (nth 2 B false) eqn:E.
    + exists (2*H+11),(H-1),21. split.
      * applys_eq (RT 2 (2*H+11)); try assumption; lia.
      * lia.
    + exists (2*H+12),(H-1),22. split.
      * applys_eq (RF 2 (2*H+12)); try assumption; lia.
      * lia.
Qed.
