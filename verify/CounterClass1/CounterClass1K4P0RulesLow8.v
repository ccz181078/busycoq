...
Record K4P0Rules8 (P:nat->nat->nat->nat->Prop)
    (P0:nat->nat->Prop):={
  k4p08_inc00_1:forall a, P0 (1+a) 1 -> P0 a 5;
  k4p08_ov:forall a b c d,
    P0 a (3+b) -> P a b c d -> P0 c (1+d);
  k4p08_ov':forall a b c d,
    P0 a (4+b) -> P a b (1+c) d -> P0 c (4+d);
  k4p08_lov2:forall a b d n,
    P0 a (5+b) -> P a b 0 (4+d) -> P 0 d 0 (1+n*2) ->
    P0 (2+n) 1;
  k4p08_lov4:forall a b d n,
    P0 a (5+b) -> P a b 0 (4+d) -> P 0 d 0 ((1+n)*2) ->
    P0 n 8
}.

Arguments k4p08_inc00_1 {P P0} _ _ _.
Arguments k4p08_ov {P P0} _ _ _ _ _ _ _.
Arguments k4p08_ov' {P P0} _ _ _ _ _ _ _.
Arguments k4p08_lov2 {P P0} _ _ _ _ _ _ _ _.
Arguments k4p08_lov4 {P P0} _ _ _ _ _ _ _ _.
...
Definition K4P0Follower8 (u z:nat):Prop:=
  exists d, 2*z+d=u+5 /\ P0 z d.

Definition K4P0Synced8 (u z:nat):Prop:=
  exists d, 2*z+d=u+4 /\ P0 z d.

Lemma k4_p08_follower_step B u x y z:
  K4Row P B true (u+2) (k4_next_x B x y) ->
  K4Row P B false (u+1) y -> z<length B ->
  (nth z B false=false -> 0<y) -> 2*length B<=u+2 ->
  K4P0Follower8 u z -> K4P0Follower8 (u+2) (k4_follow B x y z).
Proof.
  ...
  - exists (1+dt). split.
    ...
    + unfold k4_follow. rewrite Hz. cbn.
      applys_eq (k4p08_ov R0 z (d-3) (k4_next_x B x y) dt); try lia.
...
Proof.
  ...
  - destruct (k4_rows_step P R B u x y RT RF H1 H2 H3 H4 H5 H6 H7)
    ...
    eapply k4_p08_follower_step; eauto.
Qed.

Lemma k4_p08_synced_step B u x y:
  K4Row P B true u x -> K4Row P B false (u+1) y ->
  x<length B -> (nth x B false=true -> 0<x) -> 2*x<=u ->
  K4P0Synced8 u x -> K4P0Synced8 (u+2) (k4_next_x B x y).
Proof.
  ...
  - assert (0<x) by (apply Hx0; reflexivity).
    ...
    + unfold k4_next_x. rewrite Hx. cbn.
      applys_eq (k4p08_ov' R0 x (d-4) (x-1) dt); try lia.
      * replace (4+(d-4)) with d by lia. exact HP0.
      * replace (1+(x-1)) with x by lia. apply RT; try assumption; lia.
  - exists (1+df). split.
    + unfold k4_next_x. rewrite Hx. cbn. lia.
    + unfold k4_next_x. rewrite Hx. cbn.
      applys_eq (k4p08_ov R0 x (d-3) y df); try lia.
      * replace (3+(d-3)) with d by lia. exact HP0.
      * apply RF; try assumption; lia.
Qed.

Lemma k4_p08_pointer_trace B u x y n:
  K4PointerTrace B u x y n ->
  K4Row P B true u x -> K4Row P B false (u+1) y ->
  K4P0Synced8 u x -> K4P0Synced8 (u+2*n) 0.
Proof.
  intros HT. induction HT; intros RT RF HP0.
  - cbn. applys_eq HP0; lia.
  - destruct (k4_rows_step P R B u x y RT RF H H0 H1 H2 H3 H4 H5)
      as [RT' RF'].
    applys_eq (IHHT RT' ltac:(applys_eq RF'; lia)); try lia.
    eapply k4_p08_synced_step; eauto.
Qed.

Lemma k4_p08_scan_t4_reset B H S D k:
  K4ScanData P K4T4 B H S D k -> K4HeadBits K4T4 B ->
  P0 (H-2) 26 -> P0 (2*H+k+10) 1.
Proof.
  intros HD HB HP0.
  ...
  assert (HF:K4P0Follower8 (2*H+17) (H-2)).
  ...
  pose proof (k4_p08_follow_trace B _ _ _ _ _ HFT RT' RF' HF) as HF'.
  ...
  applys_eq (k4p08_lov2 R0 0 (2*(2*H+k+7)-1)
    (2*(2*H+k+7)-1) (2*H+k+8)); try lia;
    applys_eq HZ || applys_eq HZP; lia.
Qed.

Lemma k4_p08_scan_t2_reset B H S D k:
  K4ScanData P K4T2 B H S D k -> K4HeadBits K4T2 B ->
  P0 (H-1) 21 -> P0 (2*H+k+8) 8.
Proof.
  intros HD HB HP0.
  pose proof HD as [Hlen [Hhs [Hlarge [Hk [Hs [Hbase
    [Hdrop [RT [RF [Htail Hex]]]]]]]]]].
  ...
  { replace (H-1+k) with (H-1+k4_rdrops B (H-1)) by
    ...
    apply Hscan; rewrite ?Hlen; lia. }
  assert (HS0:K4P0Synced8 (2*H+15) (H-1)).
  ...
  pose proof (k4_p08_pointer_trace B _ _ _ _ HT RT' RF' HS0) as HS0'.
  ...
  assert (HBp:K4HeadBits K4T4 (k4_extend B (D+3) (2*S+k+3))).
  ...
  { replace (2*(2*H+k+7)+5) with (1+(2*(2*H+k+7)+4)) by lia.
    apply (k4p08_ov R0 0 (2*(2*H+k+7)) 0
      (2*(2*H+k+7)+4)); [|exact HT0].
    replace (3+2*(2*H+k+7)) with (2*(2*H+k+7)+3) by lia.
    exact HPend. }
  applys_eq (k4p08_lov4 R0 0 (2*(2*H+k+7))
    (2*(2*H+k+7)) (2*H+k+8)); try lia;
    applys_eq HPend' || applys_eq HT0; lia.
Qed.
