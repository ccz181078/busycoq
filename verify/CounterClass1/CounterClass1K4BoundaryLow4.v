...
Definition K4Type4Front4 (P:nat->nat->nat->nat->Prop)
...
Definition K4Type2Front4 (P:nat->nat->nat->nat->Prop)
    (B:list bool) (H S D:nat) : Prop :=
  H=2*S+D /\
  K4Row P B false (2*H+6) D /\
  P (H+1) 4 (H+1) 8 /\
  P (H+2) 2 (H+2) 6 /\
  P (H+3) 0 D (4*S+10) /\
...
Lemma k4_type4_front4_pre P B H S D:
  K4Type4Front4 P B H S D -> K4Type4PreFront P B H S D.
Proof. intros [? [? [? [? [? [? [? ?]]]]]]]; repeat split; assumption. Qed.

Lemma k4_type2_front4_pre P B H S D:
  K4Type2Front4 P B H S D -> K4Type2PreFront P B H S D.
Proof. intros [? [? [? [? [? [? ?]]]]]]; repeat split; assumption. Qed.
...
Proof.
  ...
  { exists 7. split; try lia. intros a b Hal Han Hab Haw.
    ...
    - applys_eq (k4_inc00_1 R a (b-4) (H+2)); try lia.
      ...
      + applys_eq Z; lia.
    - exact E2. }
  assert (F4:K4ExactRow P B false (2*H+4) (H+2) 4).
  { split; try lia. intros a b Hal Hab Haw.
    ...
    - applys_eq Z; lia. }
  assert (FR:K4ExactRow P B false (2*H+6) D (4*S+10)).
  { applys_eq (k4_exact_row_rov' P R B false (2*H+4)
      (H+2) 0 D (4*S+6)); try lia.
    - exact F4.
    - applys_eq C2; lia. }
  unfold K4Type2Front4. repeat split; try assumption.
  exists (4*S+10). exact FR.
Qed.

Lemma k4_low4_type2_scan_rows B H S D:
  2<=H -> length B=H+1 -> nth H B false=true ->
  D<length B -> D<>H-1 -> nth D B false=true ->
  nth (H-1) B false=true ->
  K4Type2Front4 P B H S D ->
...
Proof.
  ...
  assert (df=4*S+10) by lia. assert (dt=7) by lia. subst df dt.
  ...
  assert (B3:P (H+1) 10 H 16) by
    (applys_eq (k4_rov' R (H+1) 8 (H+1) 8 H 12); try lia;
      applys_eq B2; lia).
  assert (RF1:K4ExactRow P B false (2*H+8) (H+2) 8).
  { applys_eq (k4_exact_row_rov P R B false (2*H+6)
      D (4*S+7) (H+2) 7); try lia.
    - split; try lia. intros a b Hal Hab Haw.
      applys_eq (RF0 a b Hal Hab Haw); lia.
    - exact TD. }
  assert (RF2:K4ExactRow P B false (2*H+10) H 14).
  ...
  assert (RF3:K4ExactRow P B false (2*H+12) (H+1) 14) by
    (applys_eq (k4_exact_row_rov P R B false (2*H+10)
      H 11 (H+1) 13); try lia; assumption).
...
Lemma k4_low4_end_t4_front B H S D:
  ...
  K4EndFan P K4T4 B H S D -> K4Type4Front4 P B H S D.
Proof.
  ...
  { applys_eq (k4_exact_row_rov P R B false (2*H+8)
    ...
    - applys_eq F8; lia.
    - exact Qm1'. }
  unfold K4Type4Front4. repeat split; try assumption.
  - exists 11. exact TR.
  - exists 12. exact FR.
Qed.

Lemma k4_low4_type4_scan_rows B H S D:
  2<=H -> length B=H+1 ->
  nth H B false=true -> nth (H-1) B false=true ->
  K4Type4Front4 P B H S D ->
...
Lemma k4_low4_type4_exception B H S D:
  6<=D -> K4Type4Front4 P B H S D ->
  P (H+9) 0 (D-6) (4*S+34).
Proof.
...
Lemma k4_low4_end_t4_payload B H S D:
...
Proof.
  ...
  assert (HF:K4Type4Front4 P B H S D).
  { apply (k4_low4_end_t4_front B H S D); try assumption; try lia;
      apply Hwin; lia. }
  destruct (k4_low4_type4_scan_rows B H S D ltac:(lia)
  ...
  { apply (k4_type4_pretail P R B H S D); try assumption; try lia.
    ...
    - exact (k4_type4_front4_pre P B H S D HF). }
  ...
  assert (Hex:P (H+9) 0 (D-6) (4*S+34)) by
    (apply (k4_low4_type4_exception B H S D); assumption).
...
Lemma k4_low4_type2_exception B H S D:
  5<=D -> K4Type2Front4 P B H S D ->
  P (H+8) 0 (D-5) (4*S+30).
Proof.
...
Lemma k4_low4_end_t2_payload B H S D:
...
Proof.
  intros HS HD Hlen B0 BH BHm Hwin HE.
  assert (Hhs:H=2*S+D) by exact (proj1 HE).
  assert (HF:K4Type2Front4 P B H S D).
  { apply (k4_low4_end_t2_front B H S D); try assumption;
      apply Hwin; lia. }
  destruct (k4_low4_type2_scan_rows B H S D ltac:(lia) Hlen BH
  ...
  { apply (k4_type2_pretail P R B H S D); try assumption; try lia.
    - intros z Hz. apply Hwin. lia.
    - exact (k4_type2_front4_pre P B H S D HF). }
  ...
  assert (Hex:P (H+8) 0 (D-5) (4*S+30)) by
    (apply (k4_low4_type2_exception B H S D); assumption).
  unfold K4ScanPayload; cbn.
  split; [applys_eq RT; lia|]. split; [applys_eq RF; lia|].
  split; [intros j Hj; applys_eq (Htail j Hj); lia|].
  applys_eq Hex; lia.
