...
Definition tm : TM := Eval compute in (TM_from_str "1RB1LC_1RC---_0LD1RE_0LA1LD_0RF0RB_0LC1RE").

Notation "c --> c'" := (c -[ tm ]-> c')   (at level 40).
Notation "c -->* c'" := (c -[ tm ]->* c') (at level 40).
Notation "c -->+ c'" := (c -[ tm ]->+ c') (at level 40).

Notation "l <| r" := (l <{{D}} 1>>r) (at level 30).
Notation "l |> r" := (l<<1 {{E}}> r) (at level 30).

Notation map_length := length_map.
Notation app_length := length_app.
...
Fixpoint lowerR' (xs : list nat) : side :=
  match xs with
  | [] => const 0
  | x::xs => 1 >> [0; 1]^^x *> lowerR' xs
  end.

Definition lowerR (xs : list nat) : side :=
  match xs with
  | [] => const 0
  | x::xs => [0; 1]^^x *> lowerR' xs
  end.
...
Proof.
  ...
  evstep1.
  change (D, ((1 >> [0; 1] ^^ x *> lowerL' (xs ++ y :: z :: zs), 0, 1 >> 1 >> lowerR' []))) with
