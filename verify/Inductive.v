...
Fixpoint check_formula(ls:list N)(f:N->N)(x:N):bool :=
match ls with
| nil => true
| h::t => (f x =? h)%N && check_formula t f (N.succ x)
end.

Definition check_geoseq(ls:list N):option (N*N) :=
(match ls with
| a0::a1::t =>
  if (negb (a0 =? 0) && (a0 <? a1) && (a1 mod a0 =? 0)) then
    let k := (a1/a0) in
    if check_formula ls (fun x => (a0*(k^x))) 0 then Some (a0,k)
    else None
  else None
| _ => None
end)%N.

Definition check_constseq(ls:list N):option (N) :=
(match ls with
| a0::t =>
  if check_formula ls (fun x => a0) 0 then Some a0
  else None
| _ => None
end)%N.

Fixpoint list_diff(ls:list N):option (list N) :=
(match ls with
| a0::((a1::t1) as t) =>
  if a0 <=? a1 then
  match list_diff t with
  | Some t' => Some ((a1-a0)::t')
  | None => None
  end
  else None
...
Definition cnat_expr := N.

Inductive cseg_expr :=
| cseg_nil
| cseg_sym(a:Sym)
| cseg_block(a:list Sym)
| cseg_concat(a:cseg_expr)(b:cseg_expr)
| cseg_repeat(a:cseg_expr)(n:cnat_expr)
| cseg_arithseq(a:list ((list Sym)*Z*cnat_expr))(n:cnat_expr)
.

Inductive cside_expr :=
| cside_0inf
...
Definition cconfig_expr:Type := cside_expr*cside_expr*Q*dir.
...
Definition any_expr := {x:type_t & to_expr_type x}.
Definition make_any_expr(t:type_t)(x:to_expr_type t):any_expr.
  exists t.
  apply x.
Defined.
...
| seg_rw(a b:seg_expr)
| side_rw(a b:side_expr)
| config_rw(a b:config_expr)
| false_prop0
...
| seg_block a0,seg_block b0 => a0 == b0
...
Fixpoint side_ignore_repn(x:side_expr) :=
match x with
| side_concat x0 x1 =>
  side_concat (seg_ignore_repn x0) (side_ignore_repn x1)
| side_var i => side_var 1%positive
| _ => x
end.

Definition config_ignore_repn(x:config_expr) :=
let '(l,r,s,sgn):=x in
(side_ignore_repn l,side_ignore_repn r,s,sgn).
...
| seg_block a0,seg_block b0 => if a0 == b0 then [] else [false_prop0]
...
| seg_block a => hv3 ## (hash a)
| seg_concat a b => hv4 ## seg_hash a ## seg_hash b
| seg_repeat a n => hv5 ## seg_hash a ## nat_hash n
| seg_arithseq a n => hv6 ## list_hash (fun '(x0,x1,x2) => list_hash sym_hash x0 ## Z_hash x1 ## nat_hash x2) a ## nat_hash n
| seg_var i0 => hv7 ## Pos_hash i0
end.
...
Definition K := config_expr.
Definition K_hash := config_hash.
Definition K_eq: config_expr->config_expr->bool := eqb.
Definition K_eq_spec: forall a b:config_expr, Bool.reflect (a=b) (eqb a b) := eqb_spec.
End config_Hash.
...
Module prop_Value <: ValueType.
  Definition V:Type := prop_expr*id_t.
End prop_Value.

Module RuleMultimap := HashMultimap config_Hash prop_Value.
...
Definition expr_allFV(e:any_expr):=
match e with
...
| seg_rw a b => seg_allFV b (seg_allFV a s)
| side_rw a b => side_allFV b (side_allFV a s)
| config_rw a b => config_allFV b (config_allFV a s)
| false_prop0 => s
...
| seg_rw a b => seg_rw (simpl_seg a) (simpl_seg b)
| side_rw a b => side_rw (simpl_side a) (simpl_side b)
| config_rw a b => config_rw (simpl_config a) (simpl_config b)
...
| seg_rw a b => seg_rw (subst_seg a) (subst_seg b)
| side_rw a b => side_rw (subst_side a) (subst_side b)
| config_rw a b => config_rw (subst_config a) (subst_config b)
| false_prop0 => false_prop0
...
Definition subst_vars(mp:PositiveMap.tree any_expr)(i0:id_t)(t0:type_t):to_expr_type t0 :=
...
Definition subst_state:Type :=
  (PositiveMap.tree any_expr)*(PositiveMap.tree unit)*id_t.

Definition subst1(l1:id_t)(e:any_expr)(s:subst_state):subst_state :=
...
Definition find_subst_exgcd'(x y:id_t)(a b c:N)(s:subst_state):subst_state :=
  ...
  then
    ...
    subst1 y (make_any_expr nat_t (nat_add (nat_mul (nat_var i) (from_nat a)) (from_nat ((a-((c*y') mod a)) mod a)))) s) in

    (if (b =? 1)%N then s else
    let (i,s):=fresh_var s in
    subst1 x (make_any_expr nat_t (nat_add (nat_mul (nat_var i) (from_nat b)) (from_nat ((c*x') mod b)))) s)
  else
    s.
...
match x with
| nat_eq (nat_var i0) b => subst1 i0 (make_any_expr nat_t b) s
| nat_eq a b => if cfg.(enable_subst_exgcd) then find_subst_exgcd a b s else s
| seg_eq (seg_var i0) b => subst1 i0 (make_any_expr seg_t b) s
| side_eq (side_var i0) b => subst1 i0 (make_any_expr side_t b) s
| _ => s
end.
...
Definition cprop_merge(x1 x2:prop_expr):prop_expr :=
let (H1,G1):=x1 in
let (H2,G2):=x2 in
(H1++H2,G1++G2).
...
Definition cfollow_rule(w1 w2:prop_expr'):option (prop_expr'*_) :=
let (x1,i1):=w1 in
if negb (i1 =? 1)%positive then None else
let (x2,i2):=w2 in
let x:=(cprop_merge x1 x2) in
match prop_multistep'_trans x with
| None => None
| Some x =>
  match solve_assumptions solve_assumptions_iter_limit TrySubst.config_normal x i2 with
  | None => None
  | Some (x,mp) => Some (simpl_rule x,mp)
  end
end.
...
Fixpoint subst_list_map(mp:list (PositiveMap.tree any_expr))(i:id_t)(t:type_t):to_expr_type t :=
match mp with
| nil => mk_var i t
| mp0::mp =>
...
Definition cfollow_rule_and_get_vars(w0 w1:prop_expr'):option (prop_expr'*(list (id_t*N))) :=
match cfollow_rule w0 w1 with
| None => None
| Some (w0',mp) =>
  if negb (snd w0' =? 1)%positive then None else
  let ls := fst (N.iter (N.pred (Npos (snd w1))) (fun '(x,p) =>
    let x' :=
    match to_nat_const (subst_list_map mp p nat_t) with
    | None => x
    | Some n => (p,n)::x
    end in
    (x',Pos.succ p)) ([],xH)) in
...
Fixpoint cfollow_rules_and_get_vars(w0 w1:prop_expr')(n:nat):option (prop_expr'*(list (list (id_t*N)))) :=
match n with
| O => Some (w0,[])
| Datatypes.S n0 =>
  match cfollow_rule_and_get_vars w0 w1 with
  | None => None
  | Some (w0',ls) =>
    match cfollow_rules_and_get_vars w0' w1 n0 with
    | None => None
    | Some (w0'',ls') => Some (w0'',(rev ls)::ls')
    end
  end
end.

Fixpoint transpose_0{T}(h:list T)(t:list (list T))(x0:T):list (list T) :=
match h with
| [] => []
| hh::ht =>
  (hh::(map (hd x0) t))::transpose_0 ht (map (@tl T) t) x0
end.

Definition transpose{T}(ls:list (list T))(x0:T):list (list T) :=
match ls with
| [] => []
| h::t => transpose_0 h t x0
end.

Fixpoint fold_err{T S}(f:S->T->option S)(ls:list T)(s:S):option S :=
match ls with
| [] => Some s
| h::t =>
  match f s h with
...
Definition Nformula_to_expr(f:Nformula) :=
match f with
...
fold_err (fun mp '(i,x) =>
  match find_Nformula_v1 x with
  | None => None
  | Some f => Some (PositiveMap.add i (make_any_expr nat_t (Nformula_to_expr f)) mp)
  end) ls (PositiveMap.empty any_expr).


Definition get_vars_seq(w0 w1:prop_expr')(n:nat):option ((list (id_t*(list N)))) :=
match cfollow_rules_and_get_vars w0 w1 n with
| None => None
| Some (_,ls) =>
  let default_v := (1%positive,0%N) in
  let s := transpose ls default_v in
  let s := map (fun x => (fst (hd default_v x), map snd x)) s in
  Some s
end.
...
Definition find_IH_v2(w0 w1:prop_expr'):option (prop_expr'*prop_expr'*_*(option N)) :=
match get_vars_seq w0 w1 5 with
| None => None
| Some ls =>
  match find_Nformula find_Nformula_v1 ls with
  | None => None
  | Some mp =>
    match get_config_r w0,get_config_l w1 with
    | Some c0,Some c1 =>
      let c1' := SubstExpr.subst_config (subst_vars mp) (from_nat 0) c1 in
      let '(_,_,s,sgn):=c1 in
      Some (([],[multistep_lb_expr c0 c1' nat_ivar],1%positive),(step0_refl s sgn),[mp],None)
    | _,_ => None
    end
  end
end.
...
| from_nat can,from_nat cbn =>
  if (can <=? cbn)%N then (* inc *)
    ...
    Some (1+(cbn-(N.max dec_min bn'))/d)%N (* dec *)
| _,_ => None
end.
...
Definition find_IH_v1(x:prop_expr)(i:id_t)(x0' x0:prop_expr)(dec_min:N):option (prop_expr'*(prop_expr')*_*option N) :=
match x,x0',x0 with
| (Hx,[a]),([],[multistep'_expr _ c' _]),([],[multistep'_expr _ c _]) =>
  ...
  | Some (a,b,H) =>
    ...
    | Some (Hx',[multistep'_expr a b false],mp) =>
      ...
      let n := Visit3.visit_prop dec_min (fst w0) c' c in
      let (b',p) :=
      match n with
...
Definition find_IH x i x0' x0 (use_exp:bool)(dec_min:N) :=
match (if use_exp then find_IH_v2 (x0',1%positive) (x,i) else None) with
| Some v => Some v
| None => find_IH_v1 x i x0' x0 dec_min
end.
...
Definition d1_to_d0(d1:list Sym) :=
  repeat s0 (length d1).
...
| side_binary_inc_rule d1 qL qR QL QR =>
  forall l r n,
    l <* (d1_to_d0 d1) <* d1^^n <{{QL}} qL *> r -[ tm ]->+
    l <* d1 <* (d1_to_d0 d1)^^n <* qR {{QR}}> r
...
Definition upd_config(ls:list SetConfig)(cfg:Config):Config := {|
  ...
  dec_min := get_config (fun x => match x with | set_dec_min n => Some n | _ => None end) ls cfg.(dec_min);
...
Hypothesis tm:TM.
Hypothesis cfg:Config.
Hypothesis cfg_WF:Config_WF tm cfg.

Section subst_var.
Hypothesis mp:id_t->forall t:type_t, to_cexpr_type t.
Hypothesis mpi:cnat_expr.

Fixpoint to_cnat(x:nat_expr):cnat_expr :=
(match x with
| from_nat n => n
...
Fixpoint to_cseg(x:seg_expr):cseg_expr :=
match x with
| seg_nil => cseg_nil
| seg_sym a => cseg_sym a
| seg_block a => cseg_block a
| seg_concat a b => cseg_concat (to_cseg a) (to_cseg b)
| seg_repeat a n => cseg_repeat (to_cseg a) (to_cnat n)
| seg_arithseq a n => cseg_arithseq (map (fun '(x0,x1,x2) => (x0,x1,to_cnat x2)) a) (to_cnat n)
| seg_var i => (mp i seg_t)
end.

Fixpoint to_cside(x:side_expr):cside_expr :=
match x with
| side_0inf => cside_0inf
| side_var i => (mp i side_t)
...
let '(l,r,q,d):=x in
(to_cside l,to_cside r,q,d).

Definition to_cexpr t1 (e1:to_expr_type t1): to_cexpr_type t1.
Proof.
...
Fixpoint cseg_WF(x:cseg_expr) :=
match x with
| cseg_nil => True
| cseg_sym a => True
| cseg_block a => True
| cseg_concat a b => (cseg_WF a) /\ (cseg_WF b)
| cseg_repeat a n => (cseg_WF a)
| cseg_arithseq a n => True
end.

Fixpoint cside_WF(x:cside_expr) :=
match x with
| cside_0inf => True
...
let '(l,r,q,d):=x in
cside_WF l /\ cside_WF r.

Fixpoint seg_WFb(x:seg_expr) :=
match x with
| seg_nil => true
| seg_sym a => true
| seg_block a => true
| seg_concat a b => (seg_WFb a) && (seg_WFb b)
| seg_repeat a n => (seg_WFb a)
| seg_arithseq a n => true
| seg_var _ => false
end.

Fixpoint side_WFb(x:side_expr) :=
match x with
| side_0inf => true
...
Definition config_WFb(x:config_expr) :=
let '(l,r,q,d):=x in
side_WFb l && side_WFb r.

Lemma seg_WFb_spec x:
  seg_WFb x = true ->
  cseg_WF (to_cseg x).
Proof.
  induction x; cbn;
  repeat rewrite and_true_iff.
  all: try tauto; try congruence.
Qed.

Lemma side_WFb_spec x:
  side_WFb x = true ->
  cside_WF (to_cside x).
Proof.
  induction x; cbn;
  repeat rewrite and_true_iff; intros; repeat split.
  all: try tauto; try congruence.
  - apply seg_WFb_spec; tauto.
Qed.

Lemma config_WFb_spec x:
  config_WFb x = true ->
  cconfig_WF (to_cconfig x).
Proof.
  destruct x as [[[l r] s] sgn]; cbn.
  repeat rewrite and_true_iff; intros; repeat split.
  all: apply side_WFb_spec; tauto.
Qed.




Definition cto_nat(x:cnat_expr):N := x.

Definition seg_arithseq_entry_to_seg(n1 n2:nat)(x:(list Sym)*Z*cnat_expr):list Sym :=
let '(x0,x1,x2):=x in
let x2 := N.to_nat (cto_nat x2) in
match x1 with
...
Fixpoint cto_seg(x:cseg_expr):list Sym :=
match x with
| cseg_nil => []
| cseg_sym a => [a]
| cseg_block a => a
| cseg_concat a b => (cto_seg a) ++ (cto_seg b)
| cseg_repeat a n => (cto_seg a) ^^ (N.to_nat (cto_nat n))
| cseg_arithseq a n => seg_arithseq_to_seg a (N.to_nat (cto_nat n)) O
end.

Fixpoint binary_positive_to_side d1 n :=
match n with
| xH => d1 *> const s0
| xI n0 => d1 *> binary_positive_to_side d1 n0
| xO n0 => (d1_to_d0 d1) *> binary_positive_to_side d1 n0
end.

Definition binary_to_side d1 (n:N) :=
match n with
| N0 => const s0
| Npos n0 => binary_positive_to_side d1 n0
end.

Fixpoint binary_Pos_to_side d0 d1 d1a (n:positive) :=
match n with
| xH => d1a *> const s0
| xI n0 => d1 *> binary_Pos_to_side d0 d1 d1a n0
...
Definition cto_config(x:cconfig_expr) :=
let '(l,r,q,d):=x in
match d with
| L => (cto_side r) <{{q}} (cto_side l)
| R => (cto_side l) {{q}}> (cto_side r)
end.

Definition to_nat x := cto_nat (to_cnat x).
Definition to_seg x := cto_seg (to_cseg x).
Definition to_side x := cto_side (to_cside x).
Definition to_config x := cto_config (to_cconfig x).

Definition seg_WF x := cseg_WF (to_cseg x).
Definition side_WF x := cside_WF (to_cside x).
Definition config_WF x := cconfig_WF (to_cconfig x).


Definition to_prop0(x:prop0_expr):Prop :=
match x with
| nat_eq a b => to_nat a = to_nat b
| seg_eq a b => to_seg a = to_seg b /\ (seg_WF a <-> seg_WF b)
| side_eq a b => to_side a = to_side b /\ (side_WF a <-> side_WF b)
| config_eq a b => to_config a = to_config b /\ (config_WF a <-> config_WF b)
| seg_rw a b => seg_WF a -> (seg_WF b /\ to_seg a = to_seg b)
| side_rw a b => side_WF a -> (side_WF b /\ to_side a = to_side b)
| config_rw a b => config_WF a -> (config_WF b /\ to_config a = to_config b)
| false_prop0 => False
| multistep_expr x0 x1 n =>
  config_WF x0 ->
  (config_WF x1 /\
  (to_config x0) -[tm]->> (N.to_nat (to_nat n)) / (to_config x1))
| multistep_lb_expr x0 x1 n =>
  config_WF x0 ->
  (config_WF x1 /\
  multistep_lb tm (N.to_nat (to_nat n)) (to_config x0) (to_config x1))
| multistep'_expr x0 x1 n =>
  config_WF x0 ->
  (config_WF x1 /\
  multistep' tm n (to_config x0) (to_config x1))
end.
...
| [ H: _ |- _ ] =>
  unfold to_nat,to_seg,to_side,to_config,seg_WF,side_WF,config_WF,cto_nat in H || fail
end);
unfold to_nat,to_seg,to_side,to_config,seg_WF,side_WF,config_WF,cto_nat.
...
Proof.
  ...
  rewrite affine_map_to_Z_cons.
  destruct (from_affine_map f) as [v1 v2].
  destruct y0 as [|y0|y0]; cbn; unfold_to_xx; lia.
Qed.
...
Proof.
  ...
  - destruct x1.
    ...
    + destruct n.
      * reflexivity.
      * unfold_to_xx. cbn[simpl_nat_expr]. cbn[to_cnat]. cbn[cto_nat]. lia.
    + destruct x1_2.
      all: try reflexivity.
      cbn. lia.
  - destruct x1,x2; cbn.
    ...
    try reflexivity.
    all: unfold_to_xx; cbn[to_cnat]; cbn[cto_nat]; lia.
Qed.
...
Lemma solve_nat_eq_spec a b:
  ...
  cto_nat (to_cnat a) = cto_nat (to_cnat b).
Proof.
  cbn.
  intro E.
...
Lemma solve_seg_eq_spec a b:
  ...
  (cto_seg (to_cseg a) = cto_seg (to_cseg b) /\ (cseg_WF (to_cseg a) <-> cseg_WF (to_cseg b))).
Proof.
  cbn.
  gen b.
  induction a; intros b E; destruct b; unfold to_prop0_list in E; cbn - [eqb]; cbn - [eqb] in E.
  all: repeat (rewrite Forall_cons_iff in E; cbn in E).
  all: try tauto.
  - destruct (sym_eqb_spec a a0); split; try tauto; try congruence.
  ...
  - rewrite Forall_app in E.
    destruct E as [E1 E2].
    rw_eq_spec (IHa1 _ E1).
    rw_eq_spec (IHa2 _ E2).
    tauto.
  - rewrite Forall_app in E.
    destruct E as [E1 E2].
    rw_eq_spec (IHa _ E1).
  ...
  - rewrite Forall_app in E.
    rewrite (solve_nat_eq_spec n n0). 2: tauto.
    generalize 0.
    induction (N.to_nat (cto_nat (to_cnat n0))); intros n2.
    1: tauto.
    cbn.
    split. 2: tauto.
    f_equal.
...
Proof.
  ...
  - destruct (dir_eqb_spec d1 d2).
    + subst.
      ...
      rewrite Forall_app.
      intros [Ha Hb].
      rw_eq_spec (solve_side_eq_spec l1 l2 Ha).
      rw_eq_spec (solve_side_eq_spec r1 r2 Hb).
      tauto.
...
Proof.
  destruct x; cbn.
  1: apply solve_nat_eq_spec.
  1: apply solve_seg_eq_spec.
  1: apply solve_side_eq_spec.
  1: apply solve_config_eq_spec.
  all: unfold to_prop0_list; simpl_Forall; cbn; tauto.
Qed.
...
Lemma ExprEq_seg_spec a b:
  ...
  (cto_seg (to_cseg a) = cto_seg (to_cseg b) /\
  (cseg_WF (to_cseg a) <-> cseg_WF (to_cseg b))).
Proof.
  gen b.
  induction a; intros b; destruct b; cbn - [eqb]; try congruence;
  repeat rewrite and_true_iff.
  - tauto.
  - destruct (sym_eqb_spec a a0); try congruence; subst; tauto.
  - destruct (eqb_spec a a0); try congruence; subst; tauto.
  - intros [H1 H2].
    rw_eq_spec (IHa1 b1 H1).
    rw_eq_spec (IHa2 b2 H2).
    tauto.
  - intros [H1 H2].
    rw_eq_spec (IHa b H1).
  ...
  - intros [H1 H2].
    ...
    reflexivity.
  - destruct (eqb_spec i i0); try congruence; subst; tauto.
Qed.

Lemma ExprEq_side_spec a b:
  side_expr_eqb a b = true ->
  (cto_side (to_cside a) = cto_side (to_cside b) /\
...
Proof.
  ...
  - intros [H1 H2].
    rw_eq_spec (IHa _ H2).
    rw_eq_spec (ExprEq_seg_spec _ _ H1).
    tauto.
  - intros [H1 H2].
    erewrite ExprEq_nat_spec; eauto.
...
Lemma ExprEq_config_spec a b:
  ...
  (cto_config (to_cconfig a) = cto_config (to_cconfig b) /\
  (cconfig_WF (to_cconfig a) <-> cconfig_WF (to_cconfig b))).
Proof.
  ...
  repeat rewrite and_true_iff.
  intros [[[H0 H1] H2] H3].
  rw_eq_spec (ExprEq_side_spec _ _ H2).
  rw_eq_spec (ExprEq_side_spec _ _ H3).
...
Proof.
  ...
  try congruence.
  - intros [H1 H2].
    rewrite (ExprEq_nat_spec _ _ H1).
    rewrite (ExprEq_nat_spec _ _ H2).
    tauto.
  - intros [H1 H2].
    rw_eq_spec (ExprEq_seg_spec _ _ H1).
    rw_eq_spec (ExprEq_seg_spec _ _ H2).
    tauto.
  - intros [H1 H2].
    rw_eq_spec (ExprEq_side_spec _ _ H1).
    rw_eq_spec (ExprEq_side_spec _ _ H2).
    tauto.
  - intros [H1 H2].
    rw_eq_spec (ExprEq_config_spec _ _ H1).
    rw_eq_spec (ExprEq_config_spec _ _ H2).
    tauto.
  - tauto.
  - intros [[H1 H2] H3].
    rw_eq_spec (ExprEq_config_spec _ _ H1).
  ...
  - intros [[H1 H2] H3].
    rw_eq_spec (ExprEq_config_spec _ _ H1).
    rw_eq_spec (ExprEq_config_spec _ _ H2).
    destruct (Bool.eqb_spec n n0); try congruence.
    subst; tauto.
Qed.
...
Lemma nat_simpl_spec x:
  (to_cnat (simpl_nat x)) = (to_cnat x).
Proof.
  unfold simpl_nat.
  pose proof simpls_nat_expr_spec as H.
  unfold_to_xx.
  rewrite H.
  apply simpl_nat_expr_by_affine_map_spec.
Qed.

Ltac rw_nat_simpl_spec :=
  repeat rewrite nat_simpl_spec.

Lemma seg_simpl_spec x:
  cto_seg (to_cseg (simpl_seg x)) = cto_seg (to_cseg x).
Proof.
  induction x; cbn.
  all: rw_nat_simpl_spec.
  all: try congruence.
  - generalize 0.
    induction (N.to_nat (cto_nat (to_cnat n))); intros n2.
    1: reflexivity.
    cbn.
...
Lemma seg_simpl_WF x:
  cseg_WF (to_cseg (simpl_seg x)) <-> cseg_WF (to_cseg x).
Proof.
  induction x; cbn.
  all: rw_nat_simpl_spec.
  all: try tauto.
...
Ltac rw_seg_simpl_spec :=
  rw_nat_simpl_spec;
  repeat rewrite seg_simpl_spec;
  repeat rewrite seg_simpl_WF.

Lemma side_simpl_spec x:
  cto_side (to_cside (simpl_side x)) = cto_side (to_cside x).
Proof.
  induction x; cbn.
  all: rw_seg_simpl_spec.
  all: try congruence.
Qed.

Lemma side_simpl_WF x:
  cside_WF (to_cside (simpl_side x)) <-> cside_WF (to_cside x).
Proof.
  induction x; cbn.
  all: rw_seg_simpl_spec.
  all: try tauto.
...
Ltac rw_side_simpl_spec :=
  rw_seg_simpl_spec;
  repeat rewrite side_simpl_spec;
  repeat rewrite side_simpl_WF.

Lemma config_simpl_spec x:
  cto_config (to_cconfig (simpl_config x)) = cto_config (to_cconfig x).
Proof.
  destruct x as [[[l r] s] sgn]; cbn.
  all: rw_side_simpl_spec.
  all: try congruence.
Qed.

Lemma config_simpl_WF x:
  cconfig_WF (to_cconfig (simpl_config x)) <-> cconfig_WF (to_cconfig x).
Proof.
  destruct x as [[[l r] s] sgn]; cbn.
  all: rw_side_simpl_spec.
  all: try tauto.
...
Ltac rw_config_simpl_spec :=
  rw_side_simpl_spec;
  repeat rewrite config_simpl_spec;
  repeat rewrite config_simpl_WF.
...
Section subst_spec.
Import SubstExpr.
Hypothesis mp':id_t->forall t, to_cexpr_type t.
Hypothesis mpi':cnat_expr.
Hypothesis mp0:id_t->forall t, to_expr_type t.
Hypothesis mpi0:nat_expr.
Definition mp(i:id_t) t: to_cexpr_type t := (to_cexpr mp' mpi' t (mp0 i t)).
Definition mpi:cnat_expr := to_cexpr mp' mpi' nat_t mpi0.

Lemma nat_subst_spec x:
  (to_cnat mp' mpi' (subst_nat mp0 mpi0 x)) = (to_cnat mp mpi x).
Proof.
  induction x; cbn.
  all: rw_hyps.
  all: try reflexivity.
...
Ltac rw_nat_subst_spec :=
  rw_hyps;
  repeat rewrite nat_subst_spec.

Lemma seg_subst_spec x:
  cto_seg (to_cseg mp' mpi' (subst_seg mp0 mpi0 x)) = cto_seg (to_cseg mp mpi x).
Proof.
  induction x; cbn.
  all: rw_nat_subst_spec.
  all: try reflexivity.
  - cbn.
    generalize 0.
    induction (N.to_nat (cto_nat (to_cnat mp (to_cnat mp' mpi' mpi0) n))); intros n2.
    1: reflexivity.
    cbn.
    rewrite IHn0. clear IHn0.
    f_equal.
...
Lemma seg_subst_WF x:
  cseg_WF (to_cseg mp' mpi' (subst_seg mp0 mpi0 x)) <->
  cseg_WF (to_cseg mp (to_cnat mp' mpi' mpi0) x).
Proof.
  induction x; cbn.
  all: rw_nat_subst_spec.
  all: try reflexivity.
...
Ltac rw_seg_subst_spec :=
  rw_nat_subst_spec;
  repeat rewrite seg_subst_spec;
  repeat rewrite seg_subst_WF.

Lemma side_subst_spec x:
  cto_side (to_cside mp' mpi' (subst_side mp0 mpi0 x)) = cto_side (to_cside mp mpi x).
Proof.
  induction x; cbn.
  all: rw_seg_subst_spec.
  all: try reflexivity.
Qed.

Lemma side_subst_WF x:
  cside_WF (to_cside mp' mpi' (subst_side mp0 mpi0 x)) <->
  cside_WF (to_cside mp (to_cnat mp' mpi' mpi0) x).
Proof.
  induction x; cbn.
  all: rw_seg_subst_spec.
  all: try reflexivity.
...
Ltac rw_side_subst_spec :=
  rw_seg_subst_spec;
  repeat rewrite side_subst_spec;
  repeat rewrite side_subst_WF.

Lemma config_subst_spec x:
  cto_config (to_cconfig mp' mpi' (subst_config mp0 mpi0 x)) = cto_config (to_cconfig mp mpi x).
Proof.
  destruct x as [[[l r] s] sgn]; cbn.
  all: rw_side_subst_spec.
  all: try reflexivity.
Qed.

Lemma config_subst_WF x:
  cconfig_WF (to_cconfig mp' mpi' (subst_config mp0 mpi0 x)) <->
  cconfig_WF (to_cconfig mp (to_cnat mp' mpi' mpi0) x).
Proof.
  destruct x as [[[l r] s] sgn]; cbn.
  all: rw_side_subst_spec.
  all: try reflexivity.
...
Ltac rw_config_subst_spec :=
  rw_side_subst_spec;
  repeat rewrite config_subst_spec;
  repeat rewrite config_subst_WF.

Lemma prop0_subst_spec x:
  to_prop0 mp' mpi' (subst_prop0 mp0 mpi0 x) <-> to_prop0 mp mpi x.
Proof.
  destruct x; cbn.
  all: unfold to_nat,to_seg,to_side,to_config,seg_WF,side_WF,config_WF.
  all: rw_config_subst_spec.
  all: try tauto.
Qed.
...
Section TryInd.
Definition subst_small_vars(mp:list (PositiveMap.tree any_expr))(i:id_t)(t:type_t):to_expr_type t :=
match t as t0 return (to_expr_type t0) with
| nat_t =>
    match to_nat_const (subst_list_map mp i nat_t) with
    | Some n => if (n<?cfg.(mnc))%N then from_nat n else mk_var i _
    | None => mk_var i _
    end
| _ => mk_var i _
end.
...
if (cfg.(mnc) =? N0)%N then Some w1' else
match get_config_r w0,get_config_l w1 with
| Some c0,Some c1 =>
...
Definition subst_ind_S(w:prop_expr') (mp:list (PositiveMap.tree any_expr)):prop_expr' :=
  let w1 := w in
...
let (x1,i1):=w1 in
match FindIH.find_IH x1 i1 (fst w0') (fst w0) cfg.(enable_exp_toplevel_loop) cfg.(dec_min) with
| Some (w2,x3,mp,n) =>
...
Lemma cprop_merge_spec x1 x2:
  to_prop'' x1 ->
  to_prop'' x2 ->
  to_prop'' (cprop_merge x1 x2).
Proof.
  unfold to_prop''.
  unfold prop_merge.
  destruct x1 as [H1 G1].
  ...
  destruct G0; cbn; trivial.
{
  ...
  repeat rewrite Forall_nil_iff.
  cbn.
  unfold config_WF,to_config.
  intros H [H0 H1].
  gen H.
  rw_eq_spec H0.
  intros H.
  destruct (H H1) as [H12 [H23 _]].
  clear H.
  rewrite Nnat.N2Nat.inj_add.
  split; trivial.
  intros Ha.
  split. 1: tauto.
  eapply multistep_lb_trans.
  - apply H12,Ha.
  - apply H23,H12,Ha.
}
{
...
Proof.
  destruct x as [H G].
  cbn.
  destruct (TrySubst.list_find_subst cfg0 H (PositiveMap.empty any_expr, PositiveMap.empty unit, i)) as [[sl _] _].
  intros H0.
...
Lemma cfollow_rule_spec w1 w2:
to_prop'' (fst w1) ->
to_prop'' (fst w2) ->
match cfollow_rule w1 w2 with
| None => True
| Some (w,_) =>
  to_prop'' (fst w)
end.
Proof.
  ...
  destruct w2 as [x2 i2].
  cbn[cfollow_rule].
  destruct (negb (i1 =? 1)%positive).
  1: tauto.
  intros H1 H2.
  pose proof (cprop_merge_spec x1 x2 H1 H2) as H.
  destruct_spec prop_multistep'_trans_spec. 2: tauto.
  destruct_spec solve_assumptions_spec; trivial.
  destruct p0.
  apply simpl_rule_spec. tauto.
...
Section fext.
Hypothesis mp mp': (id_t -> forall t : type_t, to_cexpr_type t).
Hypothesis mpi: cnat_expr.
Hypothesis mp_eq: forall i t, mp i t = mp' i t.

Lemma to_nat_fext x:
  (to_cnat mp mpi x) = (to_cnat mp' mpi x).
Proof.
  induction x; cbn.
  all: congruence.
Qed.
...
Ltac solve_nat_fext :=
  rw_hyps;
  repeat rewrite to_nat_fext.

Lemma to_seg_fext x:
  cto_seg (to_cseg mp mpi x) = cto_seg (to_cseg mp' mpi x).
Proof.
  induction x; cbn.
  all: solve_nat_fext.
  all: try congruence.
  generalize 0.
  induction (N.to_nat (cto_nat (to_cnat mp' mpi n))); intro n2; cbn.
  1: reflexivity.
  rewrite IHn0. clear IHn0.
  f_equal.
...
Lemma seg_WF_fext x:
  cseg_WF (to_cseg mp mpi x) <-> cseg_WF (to_cseg mp' mpi x).
Proof.
  induction x; cbn.
  all: solve_nat_fext.
  all: try reflexivity.
...
Ltac solve_seg_fext :=
  solve_nat_fext;
  repeat rewrite to_seg_fext;
  repeat rewrite seg_WF_fext.

Lemma to_side_fext x:
  cto_side (to_cside mp mpi x) = cto_side (to_cside mp' mpi x).
Proof.
  induction x; cbn.
  all: solve_seg_fext.
  all: try reflexivity.
...
Lemma side_WF_fext x:
  cside_WF (to_cside mp mpi x) <-> cside_WF (to_cside mp' mpi x).
Proof.
  induction x; cbn.
  all: solve_seg_fext.
  all: try reflexivity.
...
Ltac solve_side_fext :=
  solve_seg_fext;
  repeat rewrite to_side_fext;
  repeat rewrite side_WF_fext.

Lemma to_config_fext x:
  cto_config (to_cconfig mp mpi x) = cto_config (to_cconfig mp' mpi x).
Proof.
  destruct x as [[[l r] s] sgn]; cbn.
  solve_side_fext.
  reflexivity.
...
Lemma config_WF_fext x:
  cconfig_WF (to_cconfig mp mpi x) <-> cconfig_WF (to_cconfig mp' mpi x).
Proof.
  destruct x as [[[l r] s] sgn]; cbn.
  solve_side_fext.
  reflexivity.
...
Ltac solve_config_fext :=
  solve_side_fext;
  repeat rewrite to_config_fext;
  repeat rewrite config_WF_fext.
...
Lemma subst_ivar_spec w n n0:
  ...
  forall mp, to_prop mp (to_cnat mp n n0) w.
Proof.
  intros H mp.
  specialize (H mp).
...
Proof.
  ...
  cbn.
  destruct sgn; cbn;
  intro; repeat split; trivial; tauto.
Qed.
...
Proof.
  ...
  intros H_.
  intro; split. 1: tauto.
  destruct (H_ H0) as [_ Hlb].
  destruct Hlb as [n' [H0' H1']].
...
Proof.
  ...
  destruct n; intro H1.
  - intro; split. 1: tauto.
    apply progress_multistep_lb; tauto.
  - intro; split. 1: tauto.
    apply evstep_multistep_lb; tauto.
Qed.
...
Definition side_0inf_block_def bsz :=
  (side_eq
  side_0inf
  (side_concat (seg_repeat (seg_block (List.repeat s0 bsz)) (from_nat 1)) side_0inf),
  1%positive).

Lemma side_0inf_block_def_spec bsz:
  to_prop0' (fst (side_0inf_block_def bsz)).
Proof.
  unfold to_prop0'.
  intros. cbn.
  split; try tauto.
...
Lemma side_concat_unfold_spec r0 r mp mpi:
  cto_side (to_cside mp mpi (side_concat_unfold r0 r)) =
  cto_seg (to_cseg mp mpi r0) *> cto_side (to_cside mp mpi r).
Proof.
...
Lemma side_concat_unfold_WF r0 r mp mpi:
  cside_WF (to_cside mp mpi (side_concat_unfold r0 r)) <->
  (cseg_WF (to_cseg mp mpi r0) /\ cside_WF (to_cside mp mpi r)).
Proof.
  induction r0; cbn; try tauto.
Qed.
...
Proof.
  ...
  split. 1: reflexivity.
  unfold side_WF.
  rewrite side_concat_unfold_WF.
  reflexivity.
Qed.

Definition repeat_S_block_def(r0:seg_expr):=
...
Ltac rw_side_concat_unfold :=
  unfold_to_xx;
  repeat rewrite side_concat_unfold_spec;
  repeat rewrite side_concat_unfold_WF; cbn;
  try tauto.

Lemma repeat_S_block_def_spec r0:
  to_prop0' (fst (repeat_S_block_def r0)).
Proof.
  unfold to_prop0'.
  intros.
  cbn.
...
Lemma from_seg_spec x0 mp mpi:
  cto_seg (to_cseg mp mpi (from_seg x0)) = x0.
Proof.
  induction x0.
  1: reflexivity.
  destruct x0.
...
Lemma from_seg_WF x0 mp mpi:
  cseg_WF (to_cseg mp mpi (from_seg x0)) <-> True.
Proof.
  induction x0.
  1: reflexivity.
  destruct x0.
...
Lemma side_concat_list_seg_WF mp mpi r0 r:
  cside_WF (to_cside mp mpi (side_concat_list_seg r0 r)) <->
  (Forall (seg_WF mp mpi) r0 /\ cside_WF (to_cside mp mpi r)).
Proof.
  induction r0; simpl_Forall; cbn.
  1: tauto.
  unfold_to_xx.
  rewrite IHr0.
  tauto.
...
Proof.
  ...
  2: congruence.
  destruct (forallb seg_WFb r3) eqn:Ewf.
  2: congruence.
  inverts E'.
  cbn.
  rewrite Nnat.N2Nat.inj_add.
  ...
  split. 2: {
    unfold_to_xx.
    rewrite side_concat_list_seg_WF.
    cbn.
    split. 2: tauto.
    intros.
    repeat split; try tauto.
    rewrite Forall_forall.
    rewrite forallb_forall in Ewf.
    intros x Ix.
    apply seg_WFb_spec,Ewf,Ix.
  }
  clear Ewf.
  f_equal.
{
...
Definition arithseq_3_def(r0:list (seg_expr*Z*N)):=
  ...
  | None => None
  | Some (r1,r2,r3,r4,p) =>
    let r123:=r1++r2++r3 in
    if forallb seg_WFb (r123) then
    Some
    (side_eq
    (side_concat (seg_arithseq r4 (from_nat 3)) r)
    (side_concat_list_seg (r123) r),
    p)
    else None
  end.
...
Proof.
  ...
  2: congruence.
  destruct (forallb seg_WFb (r1++r2++r3)) eqn:Ewf.
  2: congruence.
  inverts E'.
  cbn.
  rewrite side_concat_list_seg_spec.
...
Definition binary_0_def(d1:list Sym):=
  (side_eq
  (side_binary d1 (from_nat 0))
  side_0inf,
  1%positive).

Definition binary_mul2_def(d1:list Sym):=
  let n:=nat_var 1%positive in
  (side_eq
  (side_binary d1 (nat_mul n (from_nat 2)))
  (side_concat_unfold (from_seg (d1_to_d0 d1)) (side_binary d1 n)),
  2%positive).

Definition binary_mul2add1_def(d1:list Sym):=
  let n:=nat_var 1%positive in
  (side_eq
  (side_binary d1 (nat_add (nat_mul n (from_nat 2)) (from_nat 1)))
...
Lemma binary_0_def_spec d1:
  to_prop0' (fst (binary_0_def d1)).
Proof.
  unfold to_prop0'.
  intros.
  cbn. tauto.
...
Lemma binary_mul2_def_spec d1:
  to_prop0' (fst (binary_mul2_def d1)).
Proof.
  unfold to_prop0'.
  intros.
  cbn.
  ...
  split. 2: {
    split. 2: tauto.
    intros _.
    rewrite side_concat_unfold_WF,from_seg_WF.
    cbn.
    tauto.
  }
  rewrite side_concat_unfold_spec.
  rewrite from_seg_spec.
  cbn.
  generalize (mp0 1%positive nat_t). cbn.
  intros n.
  destruct n as [|n].
  - cbn.
    rewrite d1_to_d0_spec.
    reflexivity.
  - cbn.
    replace (n*2)%positive with (n~0)%positive by lia.
    reflexivity.
Qed.

Lemma binary_mul2add1_def_spec d1:
  to_prop0' (fst (binary_mul2add1_def d1)).
Proof.
  unfold to_prop0'.
  intros.
  cbn.
  ...
  - reflexivity.
  - cbn.
    replace (n*2+1)%positive with (n~1)%positive by lia.
    reflexivity.
Qed.
...
Proof.
  ...
  rewrite from_seg_spec,from_seg_WF.
  cbn.
  generalize (mp 1%positive nat_t). cbn. intros len.
  generalize (mp 2%positive nat_t). cbn. intros n1.
  generalize (mp 3%positive nat_t). cbn. intros n2.
  replace (N.to_nat (len+1)) with (Datatypes.S (N.to_nat len)) by lia.
  cbn.
  replace ((n1*2) mod 2)%N with 0%N by lia. cbn.
  split.
  - do 2 f_equal. lia.
...
Proof.
  ...
  replace (N.to_nat (len+1)) with (Datatypes.S (N.to_nat len)) by lia.
  cbn.
  replace ((n1*2+1) mod 2)%N with 1%N by lia. cbn.
  split.
  - do 2 f_equal. lia.
  - rewrite N.pow_add_r; cbn. lia.
Qed.
...
Definition side_binary_inc d1 qL qR QL QR :=
  ...
  (multistep'_expr
  (side_concat_unfold (from_seg qL) r,side_binary d1 n,QL,L)
  (side_concat_unfold (from_seg qR) (side_binary d1 (nat_add n (from_nat 1))),r,QR,R)
  true,
  3%positive).
...
Lemma binary_to_side_spec d1 n:
  exists n0 n1,
  binary_to_side d1 n =
  d1^^n1 *> (d1_to_d0 d1) *> binary_to_side d1 n0 /\
  binary_to_side d1 (n+1) =
  (d1_to_d0 d1)^^n1 *> d1 *> binary_to_side d1 n0.
Proof.
  destruct n as [|n].
  - exists N0 0.
    cbn.
    ...
    tauto.
  - induction n.
    + destruct IHn as [n0 [n1 [H0 H1]]].
      exists n0 (Datatypes.S n1).
      cbn.
      cbn in H0,H1.
      repeat rewrite Str_app_assoc.
      rewrite <-H0.
...
Lemma binary_Pos_to_side_spec d0 d1 d1a n:
  (exists n0 n1,
  binary_Pos_to_side d0 d1 d1a n =
  d1^^n1 *> d0 *> binary_Pos_to_side d0 d1 d1a n0 /\
  binary_Pos_to_side d0 d1 d1a (n+1) =
  d0^^n1 *> d1 *> binary_Pos_to_side d0 d1 d1a n0) \/
  (exists n1,
  binary_Pos_to_side d0 d1 d1a n =
  d1^^n1 *> d1a *> const s0 /\
  binary_Pos_to_side d0 d1 d1a (n+1) =
  d0^^n1 *> d0 *> d1a *> const s0).
Proof.
  induction n.
  - destruct IHn as [IHn|IHn].
    + destruct IHn as [n0 [n1 [H0 H1]]].
      ...
      1: reflexivity.
      do 2 f_equal. lia.
    + destruct IHn as [n1 [H0 H1]].
      right.
      exists (Datatypes.S n1).
      cbn.
      repeat rewrite Str_app_assoc.
      rewrite <-H0.
      rewrite <-H1.
...
Lemma binary_dec_to_side_inc_spec d0 d1 d1a len n:
  (n+1 < 2^len ->
  exists len0 n0 n1,
  binary_dec_to_side d0 d1 d1a (N.to_nat len) n =
  d1^^n1 *> d0 *> binary_dec_to_side d0 d1 d1a (N.to_nat len0) n0 /\
  binary_dec_to_side d0 d1 d1a (N.to_nat len) (n+1) =
  d0^^n1 *> d1 *> binary_dec_to_side d0 d1 d1a (N.to_nat len0) n0 /\
  (N.of_nat n1)+len0+1=len)%N.
Proof.
  gen n.
  ...
  cbn.
  destruct (N.eqb_spec (n mod 2) 0)%N.
  - exists len (n/2)%N 0.
    repeat split.
    + replace ((n+1) mod 2)%N with 1%N by lia. cbn.
      do 2 f_equal. lia.
    + lia.
  - rewrite N.pow_succ_r' in H.
    eassert (IH:_) by (apply (IHlen (n/2)%N); lia).
    clear IHlen.
    destruct IH as [len0' [n0' [n1' [H0 [H1 H2]]]]].
    exists len0' n0' (Datatypes.S n1').
    repeat split.
    + rewrite H0. cbn.
      rewrite Str_app_assoc.
      reflexivity.
    + replace ((n+1) mod 2)%N with 0%N by lia. cbn.
      rewrite Str_app_assoc.
      rewrite <-H1.
      do 2 f_equal. lia.
    + lia.
...
Proof.
  ...
  - cbn.
    ...
    rewrite List.app_nil_r.
    tauto.
  - do 2 rewrite Nnat.N2Nat.inj_succ. cbn.
    repeat rewrite Str_app_assoc.
    rewrite N.pow_succ_r' in H.
    replace (n mod 2)%N with 1%N by lia.
    cbn.
...
Proof.
  ...
  - intros mp mpi.
    ...
    destruct (binary_to_side_spec d1 n) as [n0 [n1 [H0 H1]]].
    rewrite H0,H1.
    intro. split. 1: tauto.
    apply H.
  - intros mp mpi.
    ...
    destruct (binary_Pos_to_side_spec d0 d1 d1a (N.succ_pos n)) as [[n0 [n1 [H0 H1]]]|[n1 [H0 H1]]].
    + rewrite H0,H1.
      intro. split. 1: tauto.
      apply H.
    + rewrite H0,H1.
  ...
  - intros mp mpi.
    ...
    generalize (mp 4%positive nat_t). cbn; intros n2.
    intro Hpre.
    split.
    1: repeat split; try lia; tauto.
    destruct (binary_dec_to_side_inc_spec d0 d1 d1a len n1) as [len' [n1' [n2' [H0 [H1 H2]]]]].
    1: lia.
    rewrite H0,H1.
    apply H.
  - intros mp mpi.
...
Lemma DH_side_to_expr_spec l mp mpi:
  cto_side (to_cside mp mpi (DH_side_to_expr l)) = l *> const s0.
Proof.
  induction l.
  1: reflexivity.
  cbn.
...
Lemma DH_config_to_expr_spec x mp mpi:
  cto_config (to_cconfig mp mpi (DH_config_to_expr x)) =
  DH_config_to_config x.
Proof.
  destruct x as [[[l r] s] sgn].
  cbn.
...
Lemma DH_side_to_expr_WF x mp mpi:
  cside_WF (to_cside mp mpi (DH_side_to_expr x)).
Proof.
  unfold DH_side_to_expr.
  rewrite side_concat_list_seg_WF.
  cbn. split; trivial.
  induction x; cbn; simpl_Forall; trivial.
  cbn; split; trivial.
Qed.

Lemma DH_config_to_expr_WF x mp mpi:
  cconfig_WF (to_cconfig mp mpi (DH_config_to_expr x)).
Proof.
  destruct x as [[[l r] s] sgn].
  cbn.
  split; apply DH_side_to_expr_WF.
Qed.

Definition DH_bounded_step(x:DH_config):DH_config :=
let '(l,r,s,sgn):=x in
match r with
| nil => x
| r0::r1 =>
  ...
  | Some (o,sgn',s') =>
    match sgn,sgn' with
    | L,L | R,R => (o::l,r1,s',sgn')
    | _,_ => (o::r1,l,s',sgn')
    end
  end
end.
Definition DH_bounded_steps(x:DH_config)(n:N):DH_config :=
N.iter n DH_bounded_step x.

Definition DH_config_to_bounded_config(x:DH_config)(l0 r0:side):Q*tape :=
let '(l,r,s,sgn):=x in
match sgn with
| L => l0 <* r <{{s}} l *> r0
| R => l0 <* l {{s}}> r *> r0
...
Lemma DH_bounded_step_spec x l_ r_:
  DH_config_to_bounded_config x l_ r_
  -[tm]->*
  DH_config_to_bounded_config (DH_bounded_step x) l_ r_.
Proof.
  destruct x as [[[l r] s] sgn].
  cbn.
  destruct r as [|r0 r].
  1: destruct sgn; constructor.
  destruct (tm (s,r0)) as [[[o sgn'] s']|] eqn:E.
  2: destruct sgn; constructor.
  destruct sgn,sgn'; cbn;
  apply progress_evstep,progress_base.
  all: constructor; apply E.
Qed.

Lemma DH_bounded_steps_spec x n l_ r_:
  DH_config_to_bounded_config x l_ r_
  -[tm]->*
  DH_config_to_bounded_config (DH_bounded_steps x n) l_ r_.
Proof.
  induction n using N.peano_ind.
  - constructor.
  - eapply evstep_trans.
...
Lemma block_step1_spec c bmaxT:
  to_prop0' (fst (block_step1 c bmaxT)).
Proof.
  pose proof (DH_bounded_steps_spec c bmaxT) as H.
  destruct c as [[[l0 r0] s0] sgn0].
  unfold to_prop0'.
  intros mp mpi.
  cbn - [eqb].
  destruct (DH_bounded_steps (l0,r0,s0,sgn0) bmaxT) as [[[l1 r1] s1] sgn1].
  destruct ((sgn0 == sgn1) && (s0 == s1) && (r1 == []) && (l0 == firstn (length r0) l1)) eqn:E.
  1: {
    eqb_cases sgn0 sgn1.
    eqb_cases s0 s1.
    eqb_cases r1 (@nil Sym).
    ...
    - remember (firstn (length r0) l1) as l1a.
      ...
      gen l r.
      assert (forall l r, r <* r0 <{{s1}} l1a *> l -[tm]->* r <{{s1}} l1a *> l1b *> l) as Hs. {
        intros.
        applys_eq H; cbn.
        rewrite <-(firstn_skipn (length r0) l1).
        rewrite Str_app_assoc.
        congruence.
      }
      induction n.
    ...
    - remember (firstn (length r0) l1) as l1a.
      remember (skipn (length r0) l1) as l1b.
      gen l r.
      assert (forall l r, l <* l1a {{s1}}> r0 *> r -[tm]->* l <* l1b <* l1a {{s1}}> r) as Hs. {
        intros.
        applys_eq H; cbn.
        rewrite <-(firstn_skipn (length r0) l1).
        rewrite Str_app_assoc.
  ...
  all: applys_eq H; cbn.
  all: rewrite <-(firstn_skipn (length r0) l1).
...
Proof.
  ...
  all: intro; split; [tauto|]; apply progress_base; constructor; apply E.
Qed.
...
| (side_rw a b,p) =>
  let (v,p):=(seg_var p,Pos.succ p) in
  (side_rw (side_concat v a) (side_concat v b),p)
| _ => x
end.
...
Definition side_check_repeat_fold_block ls :=
match ls with
...
Lemma side_check_repeat_fold_block_spec s:
  match side_check_repeat_fold_block s with
  | None => True
  | Some x => to_prop0' (fst x)
  end.
Proof.
...
Proof.
  ...
  - destruct_spec side_0inf_bsz.
    + apply side_find_repeat_fold_block_spec.
    + apply side_find_repeat_fold_dynlen_spec.
Qed.

Fixpoint side_startswith(x:side_expr)(ls:list Sym):bool :=
match ls with
| h::t =>
  match x with
  | side_concat (seg_sym x0) x1 =>
    sym_eqb x0 h && side_startswith x1 t
  | _ => false
  end
| [] => true
end.

Fixpoint side_find_binary_fold ls1 ls2 d1 :=
match ls1 with
| side_concat h1 t1 =>
  match side_find_binary_fold t1 (side_tl ls2) d1 with
  | Some v => Some (side_concat_rw_2 v)
  | None =>
    match ls2 with
    | side_binary d1' _ =>
      if list_eqb sym_eqb d1 d1' then
        if side_startswith ls1 d1 then
          Some (binary_mul2add1_def d1)
        else if side_startswith ls1 (d1_to_d0 d1) then
          Some (binary_mul2_def d1)
        else None
      else None
    | _ => None
    end
...
Fixpoint side_find_binary_Pos_fold ls1 ls2 d0 d1 d1a :=
match side_find_binary_Pos_fold_0 ls1 d0 d1 d1a with
| Some v =>
  Some v
| None =>
  match ls1 with
  | side_concat h1 t1 =>
    match side_find_binary_Pos_fold t1 (side_tl ls2) d0 d1 d1a with
    | Some v => Some (side_concat_rw_2 v)
    | None =>
      match ls2 with
      | side_binary_Pos c _ =>
...
Fixpoint side_find_binary_dec_fold ls1 ls2 d0 d1 d1a :=
match side_find_binary_dec_fold_0 ls1 d0 d1 d1a with
| Some v =>
  Some v
| None =>
  match ls1 with
  | side_concat h1 t1 =>
    match side_find_binary_dec_fold t1 (side_tl ls2) d0 d1 d1a with
    | Some v => Some (side_concat_rw_2 v)
    | None =>
      match ls2 with
...
Lemma side_find_binary_fold_spec ls1 ls2 d1:
match side_find_binary_fold ls1 ls2 d1 with
| None => True
| Some x => to_prop0' (fst x)
end.
Proof.
  gen ls2.
  induction ls1; intros ls2; cbn[side_find_binary_fold]; trivial.
  - apply binary_0_def_spec.
  - specialize (IHls1 (side_tl ls2)).
    destruct_spec side_find_binary_fold.
    + apply side_concat_rw_2_spec,IHls1.
    + destruct ls2; trivial.
      destruct (list_eqb sym_eqb d1 d0); trivial.
      destruct_spec side_startswith.
      1: apply binary_mul2add1_def_spec.
      destruct_spec side_startswith.
      1: apply binary_mul2_def_spec.  
      trivial.
Qed.

Lemma side_find_binary_Pos_fold_0_spec ls1 d0 d1 d1a:
match side_find_binary_Pos_fold_0 ls1 d0 d1 d1a with
| None => True
| Some x => to_prop0' (fst x)
end.
Proof.
  unfold side_find_binary_Pos_fold_0.
  destruct_spec side_startswith; trivial.
  destruct_spec Nat.iter; trivial.
  apply binary_Pos_1_def_spec.
Qed.

Lemma side_find_binary_Pos_fold_spec ls1 ls2 d0 d1 d1a:
match side_find_binary_Pos_fold ls1 ls2 d0 d1 d1a with
| None => True
| Some x => to_prop0' (fst x)
end.
Proof.
  gen ls2.
  induction ls1; intros ls2; cbn[side_find_binary_Pos_fold].
  all: destruct_spec side_find_binary_Pos_fold_0_spec; trivial.
  specialize (IHls1 (side_tl ls2)).
...
Lemma side_find_binary_dec_fold_0_spec ls1 d0 d1 d1a:
match side_find_binary_dec_fold_0 ls1 d0 d1 d1a with
| None => True
| Some x => to_prop0' (fst x)
end.
Proof.
  unfold side_find_binary_dec_fold_0.
  destruct_spec side_startswith; trivial.
  destruct_spec Nat.iter; trivial.
  apply binary_dec_0_def_spec.
Qed.

Lemma side_find_binary_dec_fold_spec ls1 ls2 d0 d1 d1a:
match side_find_binary_dec_fold ls1 ls2 d0 d1 d1a with
| None => True
| Some x => to_prop0' (fst x)
end.
Proof.
  gen ls2.
  induction ls1; intros ls2; cbn[side_find_binary_dec_fold].
  all: destruct_spec side_find_binary_dec_fold_0_spec; trivial.
  specialize (IHls1 (side_tl ls2)).
  ...
  + destruct ls2; trivial.
    ...
    destruct_spec side_startswith.
    1: apply binary_dec_mul2_def_spec.  
    trivial.
Qed.
...
| (side_rw l1 l2,p) =>
  let (r,p):=(side_var p,Pos.succ p) in 
  (config_rw (l1,r,s,sgn) (l2,r,s,sgn),p)
| _ => x
end.
...
| (side_rw r1 r2,p) =>
  let (l,p):=(side_var p,Pos.succ p) in 
  (config_rw (l,r1,s,sgn) (l,r2,s,sgn),p)
| _ => x
end.
...
Proof.
  ...
  intros H mp mpi.
  specialize (H mp mpi).
  destruct x; cbn; cbn in H; try tauto; try congruence.
  - split. 2: tauto.
    unfold_to_xx.
    rw_eq_spec H.
    reflexivity.
...
Definition config_find_binary_fold (c:config_expr) d1 :=
let '(l,r,s,sgn):=c in
match sgn with
| L =>
  match side_find_binary_fold r (Nat.iter (length d1) side_tl r) d1 with
  | None => None
  | Some v => Some (config_rw_r v s sgn)
  end
| R =>
  match side_find_binary_fold l (Nat.iter (length d1) side_tl l) d1 with
  | None => None
  | Some v => Some (config_rw_l v s sgn)
  end
end.

Lemma config_find_binary_fold_spec c d1:
  match config_find_binary_fold c d1 with
  | None => True
  | Some x => to_prop0' (fst x)
  end.
Proof.
  ...
  destruct sgn; cbn.
  - destruct_spec side_find_binary_fold_spec; trivial.
    apply config_rw_r_spec,H.
  - destruct_spec side_find_binary_fold_spec; trivial.
    apply config_rw_l_spec,H.
Qed.

Definition config_find_binary_Pos_fold (c:config_expr) d0 d1 d1a :=
let '(l,r,s,sgn):=c in
match sgn with
| L =>
  match side_find_binary_Pos_fold r (Nat.iter (length d1) side_tl r) d0 d1 d1a with
  | None => None
  | Some v => Some (config_rw_r v s sgn)
  end
| R =>
  match side_find_binary_Pos_fold l (Nat.iter (length d1) side_tl l) d0 d1 d1a with
  | None => None
  | Some v => Some (config_rw_l v s sgn)
  end
end.

Lemma config_find_binary_Pos_fold_spec c d0 d1 d1a:
  match config_find_binary_Pos_fold c d0 d1 d1a with
  | None => True
  | Some x => to_prop0' (fst x)
  end.
Proof.
  destruct c as [[[l r] s] sgn].
  destruct sgn; cbn.
  - destruct_spec side_find_binary_Pos_fold_spec; trivial.
    apply config_rw_r_spec,H.
  - destruct_spec side_find_binary_Pos_fold_spec; trivial.
    apply config_rw_l_spec,H.
Qed.
...
match sgn with
| L =>
  match side_find_binary_dec_fold r (Nat.iter (length d1) side_tl r) d0 d1 d1a with
  | None => None
  | Some v => Some (config_rw_r v s sgn)
  end
| R =>
  match side_find_binary_dec_fold l (Nat.iter (length d1) side_tl l) d0 d1 d1a with
  | None => None
  | Some v => Some (config_rw_l v s sgn)
  end
end.

Lemma config_find_binary_dec_fold_spec c d0 d1 d1a:
  match config_find_binary_dec_fold c d0 d1 d1a with
  | None => True
  | Some x => to_prop0' (fst x)
  end.
Proof.
  destruct c as [[[l r] s] sgn].
  destruct sgn; cbn.
  - destruct_spec side_find_binary_dec_fold_spec; trivial.
    apply config_rw_r_spec,H.
  - destruct_spec side_find_binary_dec_fold_spec; trivial.
    apply config_rw_l_spec,H.
Qed.
...
| side_0inf =>
  match cfg.(side_0inf_bsz) with
  | None => Some side_0inf_def
  | Some n => Some (side_0inf_block_def n)
  end
| side_concat ((seg_concat r0 r1) as a) r => Some (seg_concat_def a)
| side_concat (seg_repeat a (from_nat 0)) r => Some (repeat_0_def a)
| side_concat (seg_repeat a n) r =>
  match cfg.(side_0inf_bsz) with
  | None =>
    if (n == (from_nat 2)) && negb (cfg.(dec_min) == N0) then
      Some (repeat_2_def a)
    else
      Some (repeat_S_def a)
...
Proof.
  ...
  - destruct cfg.(side_0inf_bsz).
    + apply side_0inf_block_def_spec.
  ...
  - destruct a; trivial.
    ...
    + destruct n.
      ...
      apply arithseq_0_def_spec.
  - destruct n; trivial.
    destruct (n mod 2 =? 0)%N.
    + apply binary_mul2_def_spec.
    + apply binary_mul2add1_def_spec.
  - destruct n; trivial.
...
| Datatypes.S n0 =>
  match ls with
  | side_concat (seg_sym h1) t1 =>
    match side_find_repeat_unfold_limited_depth t1 n0 with
    | Some v => Some (side_concat_rw_2 v)
    | None => None
    end
  | _ => side_find_repeat_unfold ls
  end
end.

Lemma side_find_repeat_unfold_limited_depth_spec ls n:
  match side_find_repeat_unfold_limited_depth ls n with
  | None => True
  | Some x => to_prop0' (fst x)
  end.
Proof.
  gen ls.
  induction n; intros ls; cbn; trivial.
  destruct ls; cbn[side_find_repeat_unfold_limited_depth].
  all: try apply side_find_repeat_unfold_spec.
  destruct a.
  all: try apply side_find_repeat_unfold_spec.
  specialize (IHn ls).
  destruct_spec side_find_repeat_unfold_limited_depth; trivial.
  apply side_concat_rw_2_spec,IHn.
Qed.
...
Definition config_find_unfold_ex_0(x:ExtraRules)(c:config_expr):option (prop0_expr*id_t) :=
match x with
| side_binary_inc_rule d1 qL qR QL QR =>
  let '(l,r,s,sgn):=c in
    ...
    | side_binary d1' _ =>
      if list_eqb sym_eqb d1 d1' then
        if side_startswith l qL then
          Some (side_binary_inc d1 qL qR QL QR)
        else
...
match ls with
| h::t =>
  match config_find_unfold_ex_0 h c with
  | Some v => Some v
  | None => config_find_unfold_ex t c
  end
| [] => 
  match config_find_repeat_unfold c with
  | Some v => Some v
...
Lemma config_find_unfold_ex_0_spec x c:
ExtraRules_WF tm x ->
match config_find_unfold_ex_0 x c with
| None => True
| Some v => to_prop0' (fst v)
end.
Proof.
  intros H.
  pose proof (ExtraRules_WF'_spec _ H) as H'.
  destruct x.
  - cbn[config_find_unfold_ex_0].
    ...
    destruct r; trivial.
    destruct (list_eqb_spec sym_eqb d1 d0 sym_eqb_spec); trivial.
    subst.
    destruct_spec side_startswith; trivial.
...
Forall (ExtraRules_WF tm) ls ->
match config_find_unfold_ex ls c with
| None => True
| Some x => to_prop0' (fst x)
end.
Proof.
  ...
  - rewrite Forall_cons_iff.
    intros [H H0].
    cbn.
    destruct_spec (config_find_unfold_ex_0_spec);
    tauto.
Qed.

Definition config_find_init_fold_ex_0(x:ExtraRules)(c:config_expr):option (prop0_expr*id_t) :=
match x with
| side_binary_inc_rule d1 qL qR QL QR =>
...
match ls with
| h::t =>
  match config_find_init_fold_ex_0 h c with
  | Some v => Some v
  | None => config_find_init_fold_ex t c
  end
| [] =>
  None
end.

Lemma config_find_init_fold_ex_0_spec x c:
match config_find_init_fold_ex_0 x c with
| None => True
| Some v => to_prop0' (fst v)
end.
Proof.
...
match config_find_init_fold_ex ls c with
| None => True
| Some v => to_prop0' (fst v)
end.
Proof.
  ...
  1: cbn; trivial.
  cbn.
  destruct_spec config_find_init_fold_ex_0_spec; tauto.
Qed.
...
| None => None
| Some x =>
  match config_find_unfold_ex cfg.(ex_rules) x with
  | None => None
  | Some x =>
...
Definition find_init_fold x :=
match get_config_r x with
| None => None
| Some x =>
  match config_find_init_fold_ex cfg.(ex_rules) x with
  | None => None
  | Some x =>
    Some (prop0_to_prop (step0_rw_l x))
  end
...
Proof.
  ...
  destruct (get_config_r x); trivial.
  destruct_spec config_find_unfold_ex_spec; trivial.
...
Lemma find_init_fold_spec x:
to_prop' (fst x) ->
match find_init_fold x with
| None => True
| Some x =>
  to_prop' (fst x)
end.
Proof.
  intros H.
  unfold find_init_fold.
  destruct (get_config_r x); trivial.
  destruct_spec config_find_init_fold_ex_spec; trivial.
  apply prop0_to_prop_spec.
  apply step0_rw_l_spec,H0.
Qed.
...
Definition init_fold w0 :=
let w := (step0_refl' w0,w0) in
let w := rep_rw cfg.(rep_rw_limit) find_init_fold w in
w.

Lemma init_fold_spec w0:
to_prop' (fst w0) ->
let (w1,w0):=init_fold w0 in
to_prop' (fst w1) /\
to_prop' (fst w0).
Proof.
  intros H.
...
Proof.
  ...
  intro; split; trivial.
  intro; split.
  - apply DH_config_to_expr_WF.
  - applys_eq (DH_steps_from_init_spec).
    apply DH_config_to_expr_spec.
Qed.

Definition initial_steps_ex_prop:prop_expr' :=
snd (init_fold (initial_steps_prop)).

Lemma initial_steps_ex_prop_spec:
  to_prop' (fst initial_steps_ex_prop).
Proof.
  unfold initial_steps_ex_prop.
  pose proof (init_fold_spec initial_steps_prop).
  destruct (init_fold initial_steps_prop) as [w1 w0].
  cbn.
  apply H.
...
N_iter_until hlin_layers_step (inl (reset_hlin_layers initial_steps_ex_prop,true)) T.
...
Proof.
  ...
  - cbn.
    ...
    apply initial_steps_ex_prop_spec.
Qed.
...
Proof.
  ...
  tauto.
Qed.
Lemma decide_hlin_nonhalt_spec_1 T:
...
Definition follow_rule_nohyp w1 w2 :=
follow_rule w1 w2 &&& (fun w3 =>
match fst (fst w3) with
| nil => Some w3
| _ => None
end).

Definition check_follow_rule_nohyp w1 w2 :=
match follow_rule_nohyp w1 w2 with
| Some _ => true
| _ => false
end.
...
Lemma decide_hlin_nonhalt_spec cfg T:
  forall tm,
  Config_WF tm cfg ->
  decide_hlin_nonhalt tm cfg T = true ->
  ~halts tm c0.
Proof.
  intros tm H.
...
try_ind: induction
...: basic lemmas for TM execute and simpl tape
*)
