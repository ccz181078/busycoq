...
Module TM53.
Definition tm := Eval compute in (TM_from_str "1LB0LA_1RC---_0RC0RD_1LE0LB_0LE0LF_1LA1RF").
...
Definition P0 n :=
  forall m l r,
  l <* [0]^^(n*5+1) <| [0]^^(m) *> [1] *> [0]^^(n*2) *> r -->*
  l <* [1]^^(n*2+2) <* [0]^^(n*5+m+2) |> r.
...
Proof.
  ...
  remember (n*5) as n5.
  remember (n2+1) as n'.
  mid (l <* [0]^^(n5+5) <* [0]^^(n5+1) <| [0]^^(m) *> [1] *> [0]^^(n2) *> [0]^^(2+n2) *> r).
  1:{
    repeat rewrite merge_lpow.
    finish.
  }
  follow HP0.
  mid (l <* [0]^^(n5+1) <| [0]^^(2) *> [1] *> [0]^^(n2) *> [1] *> [0]^^(n5+m+2) *> [1] *> [0]^^(n2+1) *> r).
  1:{
    remember (n5+m) as v1.
    er. sr.
    er. sr.
  ...
  follow HP0.
  mid (l <* [1]^^(n2+2) <* [0]^^(n5+1) <| [0]^^((n5+m+5)) *> [1] *> [0]^^(n2) *> r).
  1:{
    remember (n5+m) as v1.
    replace (n5+2+2) with (4+n5) by lia.
    er. sr.
    er. sr.
    do 3 step1.
    finish.
...
Lemma P0_all' i:
  c0 -->*
  const 0 <* [1]^^((2^i-1)*2+2) <* [0]^^((2^i-1)*5+2+2) |> const 0.
Proof.
  eapply evstep_trans.
  2: apply P0_all.
  rewrite lpow_all0.
