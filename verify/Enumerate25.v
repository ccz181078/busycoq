...
Definition makeTM(a0 a1 a2 a3 a4 b0 b1 b2 b3 b4:option (Sym*dir*Q)):TM :=
fun '(s,m) =>
match s,m with
| A,S0 => a0
| A,S1 => a1
...
Definition node0 := Build_T (TM_from_str "1RB------------_---------------") 9 2 2.
...
Proof.
  ...
  refine (if (decide_nonhalt tm0 (CPS_LRU 1001 10 1000 2 320 2 6 0 0)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (NG 0 10 1000 2 1 2 0 true)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (NG 0 30 3000 2 0 20 1 true)) then NonHalt else _).
  refine (
    match TC_state.decide tm0 [([30000]%N,O)] with
    | (Halt h,_) => Halt h
    | (NonHalt,_) => NonHalt
    | (Unknown,_) => _
    end).
  refine (if (decide_nonhalt tm0 (CPS_LRU 1001 30 3000 2 320 0 14 2 0)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (RWL_mod 1001 30 3000 3 320 2 1 4 0)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (CPS_LRU 1001 30 3000 2 320 0 2 0 0)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (RWL_mod 1001 10 1000 2 320 2 2 6 0)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (RWL_mod 1001 100 10000 1 320 2 2 2 0)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (CPS_LRU 1001 30 3000 1 320 0 2 0 0)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (NG 0 100 10000 2 12 0 0 true)) then NonHalt else _).
  refine (
    match TC_state.decide tm0 [([300000]%N,O)] with
    | (Halt h,_) => Halt h
    | (NonHalt,_) => NonHalt
    | (Unknown,_) => _
    end).
  refine (if (decide_nonhalt tm0 (CPS_LRU 1001 300 30000 2 320 1 2 0 0)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (RWL_mod 1001 100 10000 2 320 2 3 6 0)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (RWL_mod 1001 30 3000 2 320 2 2 2 0)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (NG 0 3000 300000 2 2 4 0 true)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (RWL_mod 1001 100 10000 1 320 2 3 3 0)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (CPS_LRU 1001 30 3000 2 320 1 2 1 0)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (CPS_LRU 1001 10 1000 2 320 0 2 1 0)) then NonHalt else _).
  refine (if (decide_nonhalt tm0 (RWL_mod 1001 300 30000 1 320 2 2 4 0)) then NonHalt else _).

  refine (if (Nat.ltb 1 (get_num_undef_trans tm0)) then _ else Unknown).
  refine (
    match TC_state.decide tm0 [([10^8]%N,O)] with
    | (Halt h,_) => Halt h
    | (NonHalt,_) => NonHalt
    | (Unknown,_) => _
    end).
